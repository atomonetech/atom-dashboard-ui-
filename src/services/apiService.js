...
// src/services/ApiService.js - FIXED FOR NETWORK ACCESS

// Automatically use the host IP if not running locally, fallback to PC IP
const getCurrentHostIP = () => {
  const hostname = window.location.hostname;
  // If we are on localhost, use the PC's actual network IP (change if needed)
  if (hostname === 'localhost' || hostname === '127.0.0.1') {
    return 'http://192.168.0.34:8000'; // Replace with your PC's IP where Django is running
  }
  // If accessed via phone (e.g., 192.168.0.34), use that same IP for API
  return `http://${hostname}:8000`;
};

const API_BASE_URL = process.env.REACT_APP_API_URL || getCurrentHostIP();

...
// =================================================================================
// MACHINE COMPANY & CAPACITY MAPPING
// =================================================================================
const MACHINE_SPECS = {
  1: { company: "ISGEC", capacity: 160 },
  2: { company: "ISGEC", capacity: 160 },
  3: { company: "ISGEC", capacity: 450 },
  4: { company: "ISGEC", capacity: 300 },
  5: { company: "ISGEC", capacity: 160 },
  6: { company: "ISGEC", capacity: 300 },
  7: { company: "ISGEC", capacity: 300 },
  8: { company: "ISGEC", capacity: 300 },
  9: { company: "ISGEC", capacity: 300 },
  10: { company: "ISGEC", capacity: 160 },
  11: { company: "ISGEC", capacity: 250 },
  12: { company: "ISGEC", capacity: 250 },
  13: { company: "ISGEC", capacity: 250 },
  14: { company: "ISGEC", capacity: 250 },
  15: { company: "ISGEC", capacity: 250 },
  16: { company: "ISGEC", capacity: 250 },
  17: { company: "ISGEC", capacity: 250 },
  18: { company: "ISGEC", capacity: 250 },
  19: { company: "ISGEC", capacity: 250 },
  20: { company: "ISGEC", capacity: 250 },
  21: { company: "AIDA", capacity: 200 },
  22: { company: "AIDA", capacity: 200 },
  23: { company: "AIDA", capacity: 200 },
  24: { company: "AIDA", capacity: 200 },
  25: { company: "AIDA", capacity: 200 },
  26: { company: "AIDA", capacity: 200 },
  27: { company: "AIDA", capacity: 200 },
  28: { company: "AIDA", capacity: 200 },
  29: { company: "AIDA", capacity: 200 },
  30: { company: "AIDA", capacity: 200 },
  31: { company: "AIDA", capacity: 200 },
  32: { company: "AIDA", capacity: 200 },
  33: { company: "AIDA", capacity: 200 },
  34: { company: "AIDA", capacity: 200 },
  35: { company: "AIDA", capacity: 200 },
  36: { company: "AIDA", capacity: 200 },
  37: { company: "AIDA", capacity: 200 },
  38: { company: "AIDA", capacity: 200 },
  39: { company: "AIDA", capacity: 200 },
  40: { company: "AIDA", capacity: 200 },
  41: { company: "ISGEC", capacity: 110 },
  42: { company: "ISGEC", capacity: 110 },
  43: { company: "ISGEC", capacity: 110 },
  44: { company: "ISGEC", capacity: 63 },
  45: { company: "ISGEC", capacity: 63 },
...
export default function Plant2Live() {
  ...
  const getCurrentIdealSeconds = (machine, currentLiveSeconds = 0) => {
    ...
    const apiMode = String(machine.live_ideal_mode || "").trim().toUpperCase();
    if (machine.machine_on && apiMode === 'ONLINE') {
      return Math.max(0, Number(machine.live_ideal_time || 0));
    }
    if (!machine.machine_on) {
      const apiLiveOffline = apiMode === 'OFFLINE' ? Math.max(0, Number(machine.live_ideal_hour_time || 0)) : 0;
  ...
  const getVisibleIdealSeconds = (machine, currentLiveSeconds = 0) => {
    if (!machine) return 0;
    if (!machine.machine_on) {
      const storedOfflineSeconds = Math.max(0, Number(machine.stored_offline_ideal_shift || 0));
      const currentOfflineSeconds = Math.max(0, Number(currentLiveSeconds || 0));
  ...
  const fetchData = async () => {
    ...
    if (fetchInProgressRef.current) {
      console.log("⏭️ plant2-live fetch skipped - previous request still running");
      return;
    }
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (liveData.event_type === 'idle_reason_updated') {
            ...
            setMachines(prevMachines =>
              prevMachines.map(machine =>
                sameMachine(machine.machine_no, machineNo)
                  ? { ...machine, has_pending_reason: true }
                  : machine
              )
            );
            ...
            setSelectedMachine(prev => {
              if (prev && sameMachine(prev.machine_no, machineNo)) {
                return { ...prev, has_pending_reason: true };
              }
              return prev;
            });
          ...
          if (liveData.event_type === 'ideal_report_updated') {
            console.log("📡 IDEAL REPORT UPDATE RECEIVED ON DASHBOARD:", liveData);
            fetchData();
            return;
          }
  ...
  const handleReasonSubmit = async () => {
    ...
    const finalCategory = idleCategory === 'Other' ? customCategory.trim() : idleCategory;
    ...
    try {
      ...
      const targetMachineNo = parseInt(String(selectedMachine.machine_no).replace(/\D/g, ""), 10);

      const notificationResponse = await fetch(`${API_BASE}/api/my-notifications/`, {
        method: "GET",
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      if (!notificationResponse.ok || !notificationData.success) {
        throw new Error(notificationData.error || notificationData.message || "Could not load current notification.");
      }

      const allNotifications = Array.isArray(notificationData.data) ? notificationData.data : [];

      const currentNotifications = allNotifications.filter((notification) => {
        const machineMatch = String(notification.machine_no || "").match(/\d+/);
        const notificationMachineNo = machineMatch ? parseInt(machineMatch[0], 10) : null;
        const notificationPlant = String(notification.plant_location || notification.plant || "").trim().toLowerCase();
        const notificationMode = String(notification.ideal_mode || "").toUpperCase();
        const notificationStatus = String(notification.report_status || notification.status || "").toUpperCase();
        const isCurrentActive = notification.idle_ended_at === null || notification.idle_ended_at === "";
      ...
      if (currentNotifications.length === 0) {
        throw new Error("Current idle/offline event is not ready yet. Please wait until the machine remains idle/offline for at least 3 minutes.");
      }

      const sortedCurrentNotifications = [...currentNotifications].sort((a, b) => {
        const timeA = a.idle_started_at ? new Date(a.idle_started_at).getTime() : 0;
        const timeB = b.idle_started_at ? new Date(b.idle_started_at).getTime() : 0;
      ...
      try {
        const autoResponse = await fetch(`${API_BASE}/api/machines/${selectedMachine.machine_no}/auto-fill/?plant=2`, {
          headers: { Authorization: `Bearer ${token}` },
        });
      ...
      const response = await fetch(`${API_BASE}/api/ideal-reports/${eventId}/submit/`, {
        method: "POST",
      ...
      if (response.status === 401) {
        alert("Session expired. Please log in again to submit the reason.");
        return;
      }

      if (!response.ok || !data.success) {
        throw new Error(data.error || data.message || data.detail || `Reason submission failed (HTTP ${response.status})`);
      }
      ...
      clearNotificationsForMachine(selectedMachine.machine_no);
      ...
      if (err.name === 'AbortError') {
        alert("Backend did not respond within 15 seconds.");
      } else {
            ...
            <div key={machine.machine_no} className="card-hover" onClick={() => handleMachineClick(machine)} style={{ position: 'relative', background: 'linear-gradient(145deg, #1e293b, #0f172a)', border: `1px solid rgba(255,255,255,0.05)`, borderTop: `4px solid ${statusColor}`, borderRadius: '20px', padding: '20px', cursor: 'pointer', transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)', boxShadow: `0 8px 20px rgba(0,0,0,0.4), ${getMachineGlow(machine)}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                <div style={{ fontSize: '24px', fontWeight: '900', color: '#f8fafc', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span style={{ fontSize: '12px', color: '#fbbf24', fontWeight: '800', padding: '4px 8px', background: 'rgba(245, 158, 11, 0.1)', border: '1px solid rgba(245, 158, 11, 0.3)', borderRadius: '6px', marginLeft: '6px' }}>
                    {spec.company !== 'N/A' ? spec.company : 'N/A'}
                  </span>
                </div>
                <div style={{ fontSize: '11px', padding: '6px 12px', background: `${statusColor}20`, color: statusColor, borderRadius: '20px', fontWeight: '800', border: `1px solid ${statusColor}40` }}>

        ...
        try {
          ...
          if (liveData.event_type === "ideal_report_updated") {
            ...
            if (liveData.report_status === "SUBMITTED") {
              ...
              // ✅ IMPORTANT:
              // Update Reason Submitted state in EVERY open browser
              setReasonLoggedStates((prev) => ({
                ...prev,
                [machineNo]: submittedState,
              }));

              // Keep machine object also synchronized
              setMachines((prevMachines) =>
                prevMachines.map((machine) =>
                  sameMachine(machine.machine_no, machineNo)
                    ? {
              ...
              // If modal is open in another browser, update that also
              setSelectedMachine((prev) => {
                if (prev && sameMachine(prev.machine_no, machineNo)) {
                  return {
                    ...prev,
              ...
              // Refresh notification count in every browser
  ...
  const handleReasonSubmit = async () => {
    ...
    try {
      ...
      if (currentNotifications.length === 0) {
        throw new Error(
          "Current idle/offline event is not ready yet. Please wait until the machine remains idle/offline for at least 3 minutes.",
        );
      }
      ...
      setReasonLoggedStates((prev) => ({
        ...prev,
        [selectedMachine.machine_no]: selectedMachine.machine_on
          ? "IDLE"
          : "OFFLINE",
      }));
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    PP-
                    {String(selectedMachine.machine_no).trim().padStart(2, "0")}
                  </div>
                  ...
                  <div
                    ...
                    style={{
                      position: "relative",
                      cursor: "pointer",
                      padding: "clamp(4px, 1vw, 10px) clamp(10px, 2vw, 15px)",
                      fontSize: "clamp(10px, 2.5vw, 14px)",
            ...
            {showHistoryView ? (
                ...
                >
                  ...
                  >
                    <select
                      ...
                      style={{
                        padding:
                          "clamp(6px, 1.5vw, 10px) clamp(8px, 2vw, 15px)",
                  ...
                  <>
                    ...
                    {historyMachineMeta && (
                      <div
                        style={{
                          padding:
                            "clamp(10px, 2vw, 14px) clamp(12px, 2vw, 16px)",
                    ...
                    >
                      {historyHourlySummary.map((hour, index) => {
                        ...
                        (hour.ideal_segments || []).forEach((seg) =>
                          combinedEvents.push({
                            type: "IDEAL",
                            timeStr: seg.start_time,
                            data: seg,
                          }),
                        );
                        (hour.on_off_events || []).forEach((ev) =>
                          combinedEvents.push({
                            type: "ON_OFF",
                            timeStr: ev.time,
                            data: ev,
                        ...
                        (hour.shut_height_changes || []).forEach((ev) =>
                          combinedEvents.push({
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                ...
                                >
                                  Offline:{" "}
                                  {hour.offline_ideal_display || "0 sec"}
                                </span>
                              </div>
                            </div>
                              ...
                              {combinedEvents.map((item, i) => {
                                ...
                                if (item.type === "ON_OFF") {
                                  return (
                                    ...
                                    >
                                      {eventIcons[item.data.type] || "📌"}{" "}
                                      {item.data.time} - {item.data.title}:{" "}
                                      {item.data.details}
                                    </div>
                                  );
                                }
              ...
              >
                ...
                {isCurrentlyLunch && !selectedMachine.is_producing && (
                  ...
                  >
                    <span style={{ fontSize: "clamp(16px, 4vw, 24px)" }}>
                      🍽️
                    </span>{" "}
                    <span style={{ fontSize: "clamp(12px, 3vw, 18px)" }}>
                      LUNCH TIME BREAK
                    </span>
                  </div>
                )}
                ...
                >
                  ...
                  <div
                    style={{
                      ...
                      textShadow: `0 0 20px ${getMachineColor(
                        selectedMachine,
                      )}40`,
                      lineHeight: "1",
                    }}
                  ...
                  >
                    <div
                      style={{
                        ...
                        justifyContent: "space-between",
                        padding:
                          "clamp(8px, 2vw, 12px) clamp(10px, 2.5vw, 16px)",
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {getMachineSpec(selectedMachine.machine_no).capacity !==
                        "N/A"
                          ? `${
                              getMachineSpec(selectedMachine.machine_no)
                                .capacity
                            } TON`
                          : "N/A TON"}
                      </span>

...
//     { label: "Plant 1 Live", icon: Factory, path: "/plant1-live", color: "cyan", badge: null },
//     { label: "Plant 2 Live", icon: MonitorDot, path: "/plant2-live", color: "yellow", badge: null },
...
//     { label: "Idle Case", icon: CircleAlert, path: "/idle-case", color: "yellow", badge: null },
//     { label: "Production Report", icon: FileText, path: "/production-report", color: "indigo", badge: null },
//     { label: "QMS", icon: ShieldCheck, path: "/qms", color: "yellow", badge: null },
...
//     { label: "Analysis Hub", icon: BarChart3, path: "/analysis-hub", color: "cyan", badge: null },
//   ];
...
//   const getSidebarWidth = () => {
//     if (screenInfo.isMobile) return mobileOpen ? "200px" : "0px";
//     if (screenInfo.isIPadMini) return isCollapsed ? "72px" : "260px";
//     if (screenInfo.isIPadPro) return isCollapsed ? "80px" : "280px";
//     return isCollapsed ? "80px" : "280px";
//   };
...
//   const showToggleButton = () => {
//     if (screenInfo.isTablet && !mobileOpen) return true;
//     if (screenInfo.isDesktop) return true;
//     return false;
//   };
...
//   const getLogoWidth = () => {
//     if (screenInfo.isMobile) return "w-[80px]";
//     if (screenInfo.isTablet) return "w-[100px]";
//     return "w-[110px]";
//   };

//   const getSmallLogoWidth = () => {
//     if (screenInfo.isMobile) return "w-[40px]";
//     return "w-[50px]";
//   };

//   const toggleCollapse = () => {
//     setIsCollapsed(!isCollapsed);
//     if (isTouchDevice && "vibrate" in navigator) navigator.vibrate(50);
//   };
...
//   const getButtonPosition = () => {
//     if (screenInfo.isIPadMini) return "top-3 left-3";
//     return "top-2 left-2";
//   };
...
//         glow: "bg-blue-100/50",
//         hoverGlow: "bg-blue-50/50",
//         inactiveText: "text-slate-600 hover:text-blue-600 hover:bg-slate-50/50",
//         iconFilter: "filter drop-shadow-[0_0_2px_rgba(37,99,235,0.3)]"
//       };
//     } else {
//       const palettes = {
//         indigo: {
//           bg: "bg-gradient-to-r from-indigo-500/20 to-indigo-500/10",
//           text: "text-indigo-400",
//           border: "bg-indigo-500",
//           badge: "bg-indigo-500/20 text-indigo-400 border-indigo-500/30",
//           glow: "bg-indigo-400/60",
//           hoverGlow: "bg-gradient-to-r from-indigo-500/10 to-transparent",
//           inactiveText: "text-slate-400 hover:text-slate-200",
//           iconFilter: "filter drop-shadow-[0_0_6px_currentColor]"
//         },
//         cyan: {
...
//           badge: "bg-cyan-500/20 text-cyan-400 border-cyan-500/30",
//           glow: "bg-cyan-400/60",
//           hoverGlow: "bg-gradient-to-r from-cyan-500/10 to-transparent",
//           inactiveText: "text-slate-400 hover:text-slate-200",
//           iconFilter: "filter drop-shadow-[0_0_6px_currentColor]"
//         },
//         yellow: {
//           bg: "bg-gradient-to-r from-yellow-500/20 to-yellow-500/10",
//           text: "text-yellow-400",
//           border: "bg-yellow-500",
//           badge: "bg-yellow-500/20 text-yellow-400 border-yellow-500/30",
//           glow: "bg-yellow-400/60",
//           hoverGlow: "bg-gradient-to-r from-yellow-500/10 to-transparent",
//           inactiveText: "text-slate-400 hover:text-slate-200",
//           iconFilter: "filter drop-shadow-[0_0_6px_currentColor]"
//         }
//       };
//       return palettes[itemColor] || palettes.indigo;
//     }
//   };
...
//             return (
//               <motion.div key={item.path} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.03 }}>
//                 <motion.button
...
//                   {/* Hover Effect */}
//                   {!isActive && (
//                     <motion.div className={`absolute inset-0 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity blur-sm ${theme === 'light' ? 'bg-slate-100/60' : colors.hoverGlow}`} />
//                   )}

//                   {/* Active Glow Effect */}
//                   {isActive && showLabel && (
//                     <motion.div layoutId="activeTab" className={`absolute inset-0 rounded-lg ${colors.bg}`} transition={{ type: "spring", bounce: 0.2, duration: 0.6 }} />
//                   )}

//                   {/* Left Border Indicator */}
//                   {isActive && showLabel && (
//                     <motion.div layoutId="activeBorder" className={`absolute left-0 top-1/2 -translate-y-1/2 w-1 h-5 rounded-r-full ${colors.border}`} transition={{ type: "spring", bounce: 0.2, duration: 0.6 }} />
//                   )}
...
//           {bottomMenuItems.map((item) => {
//             const Icon = item.icon;
//             const bottomColors = theme === 'light'
...
//               <motion.button
//                 key={item.label}
//                 onClick={() => { item.path && navigate(item.path); closeSidebar(); }}
//                 whileHover={{ x: showLabel ? 3 : 0, scale: isCentered ? 1.02 : 1 }}
//                 whileTap={{ scale: 0.98 }}
...
//                 <div className="relative z-10">
//                   <motion.div
//                     animate={item.path === "/notifications" && hasNewNotification ? { rotate: [0, -10, 10, -10, 0] } : {}}
//                     transition={{ duration: 0.5, repeat: item.path === "/notifications" && hasNewNotification ? Infinity : 0, repeatDelay: 2 }}
//                   >
...
//                       </motion.span>
//                     ) : (
//                       isCentered && <div className={`absolute -top-0.5 -right-0.5 w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse shadow-lg shadow-red-500/50 ${theme === 'light' ? 'border border-white' : 'border border-[#1e293b]'}`} />
//                     )}
//                   </AnimatePresence>
//                 )}
//               </motion.button>
...
//               className={`flex items-center gap-1.5 p-1.5 rounded-lg relative overflow-hidden group cursor-pointer ${theme === 'light' ? 'bg-slate-50 border border-slate-200 hover:bg-slate-100' : 'bg-gradient-to-r from-slate-700/30 to-slate-700/20 border border-slate-700/50'}`}
//             >
//               <motion.div
//                 className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent"
//                 animate={{ x: ["-100%", "200%"] }}
...
//                 className={`relative z-10 transition-colors flex-shrink-0 ${theme === 'light' ? 'text-red-500 hover:text-red-600' : 'text-red-400 hover:text-red-300'}`}
//               >
//                 <LogOut className="w-4 h-4" />
//               </motion.button>
//             </motion.div>

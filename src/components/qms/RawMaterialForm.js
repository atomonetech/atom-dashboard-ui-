...
const RawMaterialForm = () => {
    ...
    // ==========================================
    //  1. STATE MANAGEMENT (Added 'Model' Dropdown logic)
    // ==========================================
    const [customers, setCustomers] = useState([]);
    const [parts, setParts] = useState([]);
    const [models, setModels] = useState([]); // 🔥 Naya state models ke liye
    
    const [selectedCustomer, setSelectedCustomer] = useState('');
    const [selectedPart, setSelectedPart] = useState('');
    const [selectedModel, setSelectedModel] = useState(''); // 🔥 Naya state selected model ke liye
    ...
    useEffect(() => {
        fetch(`${API_URL}/master-dropdown/?filter=customer`)
            .then(res => res.json()).then(data => setCustomers(data)).catch(()=> {
                setCustomers(['FIG', 'TATA STEEL', 'JINDAL']); 
            });
    }, []);

    // 🔥 MODIFIED: Ab Load tab hoga jab Customer, Part, AUR Model teeno aayenge
    const loadDataForGates = async (cust, part, model, dateStr) => {
        if (!cust || !part || !model || !dateStr) return;
        try {
            const opName = 'RAW MATERIAL'; 
            
            // Ab API mein Model bhi bhejenge (Agar aapne L2/L3 table mein Model ke hisab se parameter rakhe hain)
            const masterRes = await fetch(`${API_URL}/master-parameters/?customer=${encodeURIComponent(cust)}&part=${encodeURIComponent(part)}&model=${encodeURIComponent(model)}&operation=${encodeURIComponent(opName)}`);
            const masterData = await masterRes.json();
            ...
            if(allItems.length === 0) {
                // EXCT PARAMETERS FROM YOUR CONTROL PLAN SHEET
                allItems = [
                    { sr: 1, category: 'MATERIAL', item: 'APPEARANCE', spec: 'NO RUST, NO TAPER, NO DENT ETC', tol: '-', instr: 'VISUAL' },
                    { sr: 2, category: 'MATERIAL', item: 'THICKNESS', spec: '1.0', tol: '± 0.07 MM', instr: 'MICROMETER' },
                    { sr: 3, category: 'MATERIAL', item: 'WIDTH', spec: '244', tol: '± 1 MM', instr: 'MEASURING TAPE' },
                    { sr: 4, category: 'MATERIAL', item: 'LENGTH', spec: '1180', tol: '± 2 MM', instr: 'MEASURING TAPE' },
                    { sr: 5, category: 'CHEM/MECH', item: 'GRADE', spec: 'SUS 409L', tol: '-', instr: 'MILL TEST CERTIFICATE' }
                ];
            }
            setSpecList(allItems);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    useEffect(() => { loadDataForGates(selectedCustomer, selectedPart, selectedModel, selectedDate); }, [selectedCustomer, selectedPart, selectedModel, selectedDate]);
    ...
    // 🔥 HANDLE PART CHANGE (Fetches Models)
    const handlePartChange = (e) => {
        ...
        // Dummy ya original API call models lane ke liye
        if (part) fetch(`${API_URL}/master-dropdown/?filter=model&cust=${encodeURIComponent(selectedCustomer)}&part=${encodeURIComponent(part)}`).then(res => res.json()).then(data => setModels(data)).catch(()=>setModels(['K12M', 'K15C']));
    };

    // 🔥 HANDLE MODEL CHANGE (Triggers Load Data)
    ...
    const handleDeleteAction = async () => {
        ...
        if (confirmDelete) {
            try {
                alert("🗑️ Records successfully purged.");
                loadDataForGates(selectedCustomer, selectedPart, selectedModel, selectedDate);
            } catch (err) { alert("❌ Deletion failed."); }
        }
    };
    ...
    const handleSaveData = async () => {
        ...
        const payload = {
            master_data: { 
                customer: selectedCustomer, 
                part_name: selectedPart,
                model_name: selectedModel, // Model name sent to backend
                operation: 'RAW MATERIAL', 
                part_number: partNumber, 
                plant_location: activeColumn?.plant || '', 
                date: selectedDate 
    ...
    return (
        <div className="rm-page-wrapper">
            ...
            <style>{`
                .rm-page-wrapper { position: absolute; top: 0; left: 0; width: 100vw; min-height: 100vh; background-color: #f8fafc; font-family: 'Inter', 'Segoe UI', sans-serif; z-index: 10; overflow-x: hidden; }
                :root { --bg-main: #f4f7f9; --bg-card: #ffffff; --bg-input: #f8fafc; --text-main: #0f172a; --text-muted: #64748b; --border-subtle: #e2e8f0; --accent-primary: #f59e0b; --accent-hover: #d97706; --accent-glow: rgba(245, 158, 11, 0.15); --danger: #ef4444; --success: #10b981; }
                @keyframes slideUpFade { 0% { opacity: 0; transform: translateY(20px); } 100% { opacity: 1; transform: translateY(0); } }
                .animate-pop { animation: slideUpFade 0.4s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
                ...
                .form-input-light { background-color: #f8fafc; border: 1px solid #e2e8f0; color: var(--text-main); border-radius: 6px; padding: 0.5rem; width: 100%; transition: 0.2s; outline: none; font-size: 0.85rem;}
                .form-input-light:focus { border-color: var(--accent-primary); box-shadow: 0 0 0 3px var(--accent-glow); background-color: #ffffff;}
                .btn-icon-simple { background: transparent; border: none; color: #94a3b8; padding: 6px; cursor: pointer; transition: 0.2s; border-radius: 6px;}
                .btn-icon-simple:hover { color: var(--accent-primary); background: #f1f5f9;}
                ...
                .btn-outline-custom { background: #ffffff; border: 1px solid #cbd5e1; color: var(--text-main); font-weight: 700; padding: 0.4rem 1rem; border-radius: 8px;}
            `}</style>

            <nav className="dashboard-navbar">
                <div className="brand-logo" onClick={() => navigate('/qms/reports-dashboard')}>
                    <i className="bi bi-arrow-left-circle text-muted me-2" style={{fontSize: '1.2rem'}}></i> 
                    <i className="bi bi-box-seam"></i> RM Inspection
                </div>
                <div>
                    <div className="d-flex align-items-center gap-2 px-3 py-1 rounded-pill" style={{backgroundColor: '#fef3c7', border: '1px solid #fde68a'}}>
                    ...
                    <div className="card-body-custom">
                        <div className="row g-3 align-items-end">
                            ...
                            <div className="col-md-3">
                                ...
                                <select className="form-control-light" value={selectedCustomer} onChange={handleCustomerChange}>
                            ...
                            <div className="col-md-3">
                                ...
                                <select className="form-control-light" value={selectedPart} onChange={handlePartChange} disabled={!selectedCustomer}>
                            ...
                            <div className="col-md-3">
                                ...
                                <select className="form-control-light" value={selectedModel} onChange={handleModelChange} disabled={!selectedPart}>
                            ...
                            <div className="col-md-3">
                                ...
                                <div className="d-flex gap-2">
                                    <input type="text" className="form-control-light" value={partNumber} readOnly placeholder="Part No (Auto)" />
                                </div>
                            </div>
                        </div>
                    </div>
                                    ...
                                    {activeColId && activeColumn && (
                                        <div className="meta-strip-light">
                                            <div className="row g-3 align-items-center">
                                                ...
                                                <div className="col-md-3"><label>Heat / Lot No</label><input type="text" className="form-control-light" placeholder="Heat ID" value={activeColumn.machine || ''} onChange={(e) => handleActiveColChange('machine', e.target.value)} /></div>
                                                <div className="col-md-3"><label>Data Format</label>
                                                    <select className="form-control-light" value={activeColumn.entryFormat || 'single'} onChange={(e) => handleActiveColChange('entryFormat', e.target.value)}>
                                                        <option value="single">Single Value</option>
                                                        <option value="dual">Reading 1 & 2</option>
                                                    </select>
                                                </div>

...
const BinTrollingForm = () => {
  ...
  useEffect(() => {
    if (id) {
      const fetchReportData = async () => {
        setLoading(true);
        try {
          const response = await axios.get(`${API_BASE_URL}/api/get-single-production-report/bin-trolley/${id}/`);
          
          if (response.data.success) {
            const data = response.data.data;
            setIsAlreadyFilled(true); // Disable editing
            ...
            setCheckpointData((prev) => {
              ...
              Object.keys(cpData).forEach((key) => {
                updated[key] = {
                  checkingMethod: cpData[key]?.checkingMethod || prev[key]?.checkingMethod,
          ...
          if (weekData.checkpoints) {
            setCheckpointData((prev) => {
              ...
              Object.keys(weekData.checkpoints).forEach((id) => {
                updated[id] = {
                  checkingMethod: weekData.checkpoints[id].checkingMethod || prev[id]?.checkingMethod,
  ...
  const resetForm = () => {
    ...
    setCheckpointData(
      checkpoints.reduce((acc, cp) => {
        acc[cp.id] = { checkingMethod: cp.defaultMethod, status: "", remarks: "" };
        return acc;
      }, {}),
    );
    setCleaningData(
  ...
  const handleBack = () => {
    window.history.back(); // Changed to history.back so it returns to notifications cleanly
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden border-0">
          <div className="bg-gradient-to-r from-white to-cyan-300 px-6 py-5">
            <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-3">
              <h1 className="text-2xl font-bold tracking-tight text-[#048595]">
                {id ? "Bin & Trolley Report (REVIEW)" : "Bin & Trolley Cleaning Check Sheet And Maintenance"}
              </h1>
            ...
            <div className="mx-6 mt-5 flex items-start gap-3 bg-green-50 border border-green-300 rounded-xl px-5 py-4">
              ...
              <div>
                ...
                <p className="text-green-600 text-sm mt-0.5">
                  Is week ka data pehle se fill hai. Neeche read-only mode mein dekh sakte ho.
                </p>
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-8">
            ...
            <div className={`bg-gradient-to-br from-white to-[#f8fdff] rounded-xl p-5 border shadow-sm ${isAlreadyFilled ? "border-green-200" : "border-[#b2ebf2]"}`}>
              <div className={`flex justify-between items-center pb-2 mb-4 border-b ${isAlreadyFilled ? "border-green-400" : "border-cyan-500"}`}>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className={isAlreadyFilled ? "bg-green-50" : "bg-[#e0f7fa]"}>
                  ...
                  <tbody>
                    {checkpoints.map((cp, index) => (
                      <tr key={cp.id} className={isAlreadyFilled ? "bg-green-50/30" : "hover:bg-[#f8fdff]"}>
                        ...
                        <td className="border border-[#b2ebf2] p-3">
                          {isAlreadyFilled ? (
                            <div className={readonlyInput}>{checkpointData[cp.id]?.checkingMethod || "-"}</div>
                          ) : (
                            <select
                        ...
                        <td className="border border-[#b2ebf2] p-3">
                          {isAlreadyFilled ? (
                            <div className={`${readonlyInput} text-center font-bold ${
                                checkpointData[cp.id]?.status === "Y" ? "text-green-600" : checkpointData[cp.id]?.status === "N" ? "text-red-500" : ""
                              }`}
                            >
                        ...
                        <td className="border border-[#b2ebf2] p-3">
                          {isAlreadyFilled ? (
                            <div className={readonlyInput}>{checkpointData[cp.id]?.remarks || "-"}</div>
                          ) : (
                            <input
                              type="text"
                ...
                <table className="w-full text-sm border-collapse">
                  ...
                  <tbody>
                    {cleaningDetails.map((detail) => (
                      <tr key={detail.id} className={isAlreadyFilled ? "bg-green-50/30" : "hover:bg-[#f8fdff]"}>
                        <td className="border border-[#b2ebf2] p-3 text-slate-700">{detail.label}</td>
                        <td className="border border-[#b2ebf2] p-3">
                          {isAlreadyFilled ? (
                            <div className={readonlyInput}>{cleaningData[detail.id]?.frequency || "-"}</div>
                          ) : (
                            <input
                              type="number"
                              placeholder="Qty"
                        ...
                        <td className="border border-[#b2ebf2] p-3">
                          {isAlreadyFilled ? (
                            <div className={readonlyInput}>{cleaningData[detail.id]?.remarks || "-"}</div>
                          ) : (
                            <input
                              type="text"
                              placeholder="Enter remarks"
                  ...
                  <tbody>
                    {maintenanceDetails.map((detail) => (
                      <tr key={detail.id} className={isAlreadyFilled ? "bg-green-50/30" : "hover:bg-[#f8fdff]"}>
                        ...
                        <td className="border border-[#b2ebf2] p-3">
                          {isAlreadyFilled ? (
                            <div className={readonlyInput}>{maintenanceData[detail.id]?.frequency || "-"}</div>
                          ) : (
                            <input
                              type="number"
                              placeholder="Qty"
                        ...
                        <td className="border border-[#b2ebf2] p-3">
                          {isAlreadyFilled ? (
                            <div className={readonlyInput}>{maintenanceData[detail.id]?.remarks || "-"}</div>
                          ) : (
                            <input
                              type="text"
                              placeholder="Enter remarks"
            ...
            {/* Form Actions (Conditional Rendering based on ID vs Weekly Check) */}
            {id ? (
              // 🔥 If accessed via Notification (View Mode) -> Show APPROVE BUTTON
              <div className="flex justify-between items-center mt-6">
                <div className="flex flex-col">
                  ...
                  <input
                    ...
                    value={preparedBy}
                    readOnly
                    className="px-3 py-2 border border-[#b2ebf2] rounded-lg bg-[#f0fffe] text-sm text-slate-700 w-full sm:w-64 cursor-not-allowed"
                  />
                </div>
                <button
                  type="button"
                  onClick={() => alert("Report Approved Successfully! (Implement API here)")}
                  className="px-6 py-2.5 bg-[#10b981] hover:bg-[#059669] text-white font-bold rounded-lg transition-all text-sm shadow-sm flex items-center gap-2"
                >
                  <Check size={16} /> APPROVE REPORT
                </button>
              </div>
            ) : isAlreadyFilled ? (
              // If already filled via standard navigation -> Show BACK button
              ...
              // Normal Create Mode -> Show RESET & SAVE buttons

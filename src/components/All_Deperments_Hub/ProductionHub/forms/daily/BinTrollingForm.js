...
const BinTrollingForm = () => {
  ...
  const handleCheckpointChange = (id, field, value) => {
    ...
    setCheckpointData(prev => ({ ...prev, [id]: { ...prev[id], [field]: value } }));
  };

  const handleCleaningChange = (id, field, value) => {
    if (isAlreadyFilled) return;
    setCleaningData(prev => ({ ...prev, [id]: { ...prev[id], [field]: value } }));
  };

  const handleMaintenanceChange = (id, field, value) => {
    if (isAlreadyFilled) return;
    setMaintenanceData(prev => ({ ...prev, [id]: { ...prev[id], [field]: value } }));
  };

  const resetForm = () => {
    if (isAlreadyFilled) return;
    setCheckpointData(checkpoints.reduce((acc, cp) => {
      acc[cp.id] = { checkingMethod: cp.defaultMethod, status: '', remarks: '' };
      return acc;
    }, {}));
    setCleaningData(cleaningDetails.reduce((acc, cd) => {
      acc[cd.id] = { frequency: '', remarks: '' };
      return acc;
    }, {}));
    setMaintenanceData(maintenanceDetails.reduce((acc, md) => {
      acc[md.id] = { frequency: '', remarks: '' };
      return acc;
    }, {}));
  };
  ...
  const readonlyInput = "w-full border border-[#b2ebf2] rounded px-2 py-1.5 text-sm text-slate-700 bg-[#f0fffe] cursor-not-allowed";
  const readonlySelect = "w-full border border-[#b2ebf2] rounded px-2 py-1.5 text-sm text-slate-700 bg-[#f0fffe] cursor-not-allowed pointer-events-none";
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#f8fdff] to-[#f0fbfe] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600 font-medium">Checking existing data...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden border-0">
          ...
          {isAlreadyFilled && (
            <div className="mx-6 mt-5 flex items-start gap-3 bg-green-50 border border-green-300 rounded-xl px-5 py-4">
              ...
              <div>
                <p className="font-bold text-green-700 text-base">Data Already Filled for {currentWeek}!</p>
                  ...
                  <tbody>
                    {checkpoints.map((cp, index) => (
                      <tr key={cp.id} className={isAlreadyFilled ? 'bg-green-50/30' : 'hover:bg-[#f8fdff]'}>
                        ...
                        <td className="border border-[#b2ebf2] p-3">
                          {isAlreadyFilled ? (
                            <div className={`${readonlyInput} text-center font-bold ${checkpointData[cp.id]?.status === 'Y' ? 'text-green-600' : checkpointData[cp.id]?.status === 'N' ? 'text-red-500' : ''}`}>

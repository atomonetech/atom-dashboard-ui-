...
const MChangeTrackForm = () => {
      ...
      const fetchReportData = async () => {
        try {
          const response = await axios.get(
            `${BASE_URL}/api/get-single-production-report/four-m-record/${id}/`
          );
          ...
          if (response.data.success) {
            ...
            setFormData({
              ...
              natureOfChange:
                d.natureOfChange || d.nature_of_change || "Planned",
              actionTaken: d.actionTaken || d.action_taken || "",
              partNameNo: d.partInfo || d.part_info || "",
              operationNo: d.operationNo || d.operation_no || "",
              trainingProvided:
                d.approvalTraining?.trainingProvided ||
                d.training_provided ||
                "",
              setupApproval: {
                status:
                  d.approvalTraining?.setupApproval || d.setup_approval || "",
              },
              retroactive: {
            ...
            if (
              d.retro_qty_checked !== null ||
              d.retro_qty_ok !== null ||
              d.retro_entry_qty !== null
            ) {
              setActiveAction("retroactive");
            } else if (
              d.cont_qty_checked !== null ||
              d.cont_qty_ok !== null ||
              d.cont_entry_qty !== null
            ) {
  ...
  const getNextStatus = (currentStatus) => {
    if (!currentStatus || currentStatus === "NOT_SET" || currentStatus === "") {
      return "CHANGE";
    }
  ...
  const getStatusDot = (status) => {
    ...
    if (status === "CHANGE") {
      return (
        <span
          className={`inline-block w-7 h-7 rounded-full bg-rose-500 transition-all shadow-sm ${disableClass}`}
    ...
    if (status === "OK" || status === "NO_CHANGE" || status === "NO CHANGE") {
      return (
        <span
          className={`inline-block w-7 h-7 rounded-full bg-emerald-500 transition-all shadow-sm ${disableClass}`}
          title="No Change"
        ></span>
      );
    }
    ...
    return (
      <span
        className={`inline-block w-7 h-7 rounded-full bg-slate-200 transition-all shadow-sm ${disableClass}`}
        title="Not Set"
      ></span>
    );
  };
  ...
  // ✅ APPROVE FUNCTION FOR VIEW MODE
  ...
  const handleSubmit = async () => {
    ...
    const finalized4MData = {
      man:
        !currentDayData.man || currentDayData.man === ""
          ? "NOT_SET"
          : currentDayData.man,
      machine:
        !currentDayData.machine || currentDayData.machine === ""
          ? "NOT_SET"
          : currentDayData.machine,
      material:
        !currentDayData.material || currentDayData.material === ""
          ? "NOT_SET"
          : currentDayData.material,
      method:
        !currentDayData.method || currentDayData.method === ""
          ? "NOT_SET"
    ...
    const payload = {
      ...
      retro_qty_checked:
        activeAction === "retroactive"
          ? parseNumOrNull(formData.retroactive.qtyChecked)
          : null,
      retro_entry_qty:
        activeAction === "retroactive"
          ? parseNumOrNull(formData.retroactive.entryQty)
          : null,
      retro_qty_ok:
        activeAction === "retroactive"
          ? parseNumOrNull(formData.retroactive.qtyOk)
          : null,
      retro_rw:
        activeAction === "retroactive"
          ? parseNumOrNull(formData.retroactive.rw)
          : null,
      retro_scrap:
        activeAction === "retroactive"
          ? parseNumOrNull(formData.retroactive.scrap)
          : null,

      cont_qty_checked:
        activeAction === "containment"
          ? parseNumOrNull(formData.containmentSuspected.qtyChecked)
          : null,
      cont_entry_qty:
        activeAction === "containment"
          ? parseNumOrNull(formData.containmentSuspected.entryQty)
          : null,
      cont_qty_ok:
        activeAction === "containment"
          ? parseNumOrNull(formData.containmentSuspected.qtyOk)
          : null,
      cont_rw:
        activeAction === "containment"
          ? parseNumOrNull(formData.containmentSuspected.rw)
          : null,
      cont_scrap:
        activeAction === "containment"
          ? parseNumOrNull(formData.containmentSuspected.scrap)
          : null,
  ...
  const getFormattedDate = () => {
    return new Date(headerDate).toLocaleDateString("en-US", {
      month: "short",
      day: "numeric",
      year: "numeric",
    });
  ...
  const inputStyle =
    "w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all bg-white text-slate-800 disabled:bg-slate-50 disabled:text-slate-400 disabled:cursor-not-allowed";
      ...
      >
        <div className="max-w-5xl mx-auto">
          <div className="flex justify-between items-center mb-6">
            ...
            {id && (
              <span
                className={`text-xs font-bold px-3 py-2 rounded-lg uppercase tracking-wider ${
                  ...
                  <div className="p-4">
                    <div className="grid grid-cols-2 gap-3">
                      {categories.map((category) => (
                        <div
                          key={category.id}
                          className="flex items-center justify-between p-3 bg-slate-50/60 rounded-xl border border-slate-100"
                        >
              ...
              <div className="border-t border-slate-100 pt-8">
                ...
                <div className="mb-5">
                  ...
                  <textarea
                    ...
                    readOnly={!!id}
                    onChange={(e) =>
                      handleFormChange("changeDesc", e.target.value)
                    }
                    className={inputStyle}
                    rows="3"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
                  <div className="relative">
                    ...
                    <div
                      ...
                      } text-slate-800 flex justify-between items-center transition-all`}
                      onClick={() => {
                        if (!id) setShowPartDropdown(!showPartDropdown);
                      }}
                    >
                      <span
                        className={
                      ...
                      <div className="absolute z-50 w-full mt-1.5 bg-white border border-slate-200 rounded-lg shadow-xl max-h-[320px] flex flex-col overflow-hidden">
                        ...
                        <ul className="overflow-y-auto flex-1 py-1">
                          ...
                          {partsList
                            ...
                            .map((part, index) => {
                              const displayName = `${part.part_name} ${
                                part.part_no ? `(${part.part_no})` : ""
                              }`;
                  ...
                  <div>
                    ...
                    >
                      ...
                      {id &&
                        formData.operationNo &&
                        !operationList.includes(formData.operationNo) && (
                          <option value={formData.operationNo}>
                            {formData.operationNo}
                          </option>
                        )}
                    </select>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
                    <div
                      className={`p-5 rounded-xl border transition-all duration-200 ${
                        activeAction === "retroactive"
                          ? "bg-white border-red-500 ring-4 ring-red-500/10 shadow-sm"
                    ...
                    >
                      <div className="flex items-center gap-2.5 mb-4 pb-2 border-b border-slate-100">
                        ...
                        <label
                          htmlFor="radio-retro"
                          className={`font-bold text-xs uppercase tracking-wider w-full ${
                            id
                              ? "text-slate-500"
                      ...
                      <div className="space-y-3">
                        <div>
                          ...
                          >
                            ...
                            {id &&
                              formData.dispatchDetail.customer &&
                              !customerList.includes(
                                formData.dispatchDetail.customer
                              ) && (
                                <option
                                  value={formData.dispatchDetail.customer}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="flex flex-col">
                        ...
                        <input
                          ...
                          className="px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500/20 text-sm w-full bg-white text-slate-800 disabled:bg-slate-50 disabled:text-slate-500"
                        />
                      </div>
                      ...
                      {id && approvedBy && (
                        <div className="flex flex-col">
                          ...
                          <input
                            ...
                            value={approvedBy}
                            readOnly
                            className="px-3 py-2 border border-slate-200 rounded-lg text-sm w-full bg-slate-50 text-slate-500"
                          />
                        </div>
                      )}

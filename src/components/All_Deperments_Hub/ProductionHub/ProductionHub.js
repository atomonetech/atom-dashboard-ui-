...
const ProductionHub = () => {
    ...
    const navigateToForm = (id, title) => {
        switch (id) {
            ...
            case 12: navigate('/Good-Receipt'); break;
    ...
    // ✅ AB SABHI CARDS PE MODAL AAYEGA
    ...
    return (
        <div className="hub-wrapper">
            ...
            <style>{`
                ...
                .modal-content { background: white; border-radius: 20px; padding: 24px; max-width: 400px; width: 100%; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.25); }
                .modal-header { margin-bottom: 20px; padding-bottom: 12px; border-bottom: 1px solid #e2e8f0; }
                .modal-header h3 { font-weight: 700; color: #0f172a; margin: 0; font-size: 1.25rem; }
                .option-btn { width: 100%; padding: 14px 16px; margin: 8px 0; border: none; border-radius: 12px; font-size: 0.95rem; font-weight: 600; cursor: pointer; transition: all 0.2s ease; display: flex; align-items: center; justify-content: center; gap: 12px; }
                ...
                .close-btn { background: #ef4444; color: white; margin-top: 8px; }
                ...
                <div className="row g-4">
                    {productionReports.map((r) => (
                        <div className="col-md-6 col-lg-4" key={r.id}>
                            {/* ✅ r pass kiya cardData ke liye */}
                            <div className="card-custom" onClick={() => handleCardClick(r.id, r.title, r)}>

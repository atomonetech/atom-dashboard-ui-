          ...
          const fetchReport = async () => {
            try {
              const res = await axios.get(
                `${API_BASE_URL}/api/get-single-maintenance-report/drill-machine/${id}/`
              );
              ...
              if (res.data.success) {
                ...
                setMetaData({
                  machineName: meta.machineName || 'Drill Machine', // Default value if not provided
                  date: meta.date || '',
                ...
                errorAlert(res.data.error || "Failed to load Drill Check Sheet.");
              }
            } catch (err) {
              console.error("Error loading Check Sheet:", err);
              errorAlert(err.response?.data?.error || "Failed to load Drill Check Sheet.");
            }
          };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        successAlert("✨ Drill record saved successfully!");
    ...
    >
      <style>{`
        ...
        * { font-family: 'Inter', sans-serif; }
        ...
        .btn-save:hover:not(:disabled) { background: #5b21b6; box-shadow: 0 5px 15px rgba(109, 40, 217, 0.4); }

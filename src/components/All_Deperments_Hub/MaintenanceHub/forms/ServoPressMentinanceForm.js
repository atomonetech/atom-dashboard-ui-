...
const ServoPressMaintenanceForm = () => {
            ...
            const fetchReport = async () => {
              try {
                const res = await axios.get(
                  `${API_BASE_URL}/api/get-single-maintenance-report/servo-press/${id}/`
                );
                ...
                if (res.data.success) {
                  ...
                  setMetaData({
                    machineName: meta.machineName || 'Servo Press', // Default value if not provided
                    date: meta.date || '',
                  ...
                  errorAlert(res.data.error || "Failed to load Servo Press Check Sheet.");
                }
              } catch (err) {
                console.error("Error loading Check Sheet:", err);
                errorAlert(err.response?.data?.error || "Failed to load Servo Press Check Sheet.");
              }
            };
  ...
  const handleSubmit = async (e) => {
    ...
    setIsSubmitting(true);
    const currentUser = localStorage.getItem("username") || "Unknown User"; // <--- Get current user's username

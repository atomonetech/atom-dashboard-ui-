...
const PokaYokeChecksheet = () => {
  ...
  const currentFormKey = formKey || "poka-yoke";
  ...
  useEffect(() => {
    const fetchViewReport = async () => {
      ...
      try {
        const response = await axios.get(
          `${API_BASE_URL}/api/get-single-maintenance-report/${currentFormKey}/${currentReportId}/`,
        );

        console.log("🔥 POKA YOKE VIEW RESPONSE =", response.data);
        ...
        if (response.data?.success) {
          ...
          const loadedChecks = (data.check_points || []).map((cp) => ({
            id: cp.s_no,
            detailEng: cp.poka_yoke_detail || "",
            detailHin: "",
            method: cp.checking_method || "",
            status: cp.is_ok ? "OK" : "NG",
            remarks: cp.remarks || "",
          }));
    ...
    fetchViewReport();
  }, [isViewMode, currentFormKey, currentReportId, todayDate]);
  ...
  const resetForm = () => {
    ...
    setChecklist(
      initialChecks.map((item) => ({ ...item, status: "", remarks: "" })),
    );
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      department_name: `${selectedPlant} (Maintenance)`,
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/poka-yoke/save/`, {
        method: "POST",
        headers: {
      ...
      if (response.ok && result.success) {
        ...
        setExistingDataList((prev) => [
          {
            id: result.record_id || result.id || Date.now(),
            date: selectedDate,
            plant_name: selectedPlant,
            machine_no: selectedMachine,
            checked_by_maintenance: signatures.checkedBy,
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-gray-50 rounded-lg p-3">
                ...
                <div className="font-semibold text-gray-900">
                  {selectedExistingData.checked_by_maintenance ||
                    "Not provided"}
                </div>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-3">
                ...
                <div className="font-semibold text-gray-900">
                  {selectedExistingData.verified_by_production ||
                    "Not provided"}
                </div>
              </div>
            </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-['Inter',system-ui]">
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14 sm:h-16">
            <button
              type="button"
              onClick={handleBack}
              className="flex items-center gap-2 cursor-pointer group min-w-0 flex-1 text-left"
            >
              ...
              <span className="font-extrabold text-emerald-600 text-sm sm:text-base truncate">
                {isViewMode ? "Back to Notifications" : "Poka Yoke Monitoring"}
              </span>
            </button>
          </div>
        </div>
      ...
      <div className="pt-20 sm:pt-24 pb-8 px-4 sm:px-6 lg:px-8 max-w-[1400px] mx-auto">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-4 sm:p-6 mb-6 shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="relative">
              ...
              <select
                className="w-full bg-slate-50 border border-gray-300 rounded-lg px-3 py-2 text-sm font-medium focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition disabled:opacity-50 text-slate-700 disabled:bg-slate-100 disabled:cursor-not-allowed"
        ...
        {showWarning && selectedMachine && !isViewMode && (
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-red-600 text-xs mt-1">
                  This machine&apos;s checksheet has already been submitted for
                  today. You cannot submit again.
                </p>
              </div>
            </div>
          </div>
                  ...
                  {checklist.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50 transition">
                      ...
                      <td className="px-3 sm:px-4 py-3">
                        <input
                          type="text"
                          className="w-full bg-slate-50 border border-gray-300 rounded-lg px-2 py-1.5 text-xs text-slate-700 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-200 outline-none transition read-only:bg-slate-100"
        ...
        {selectedMachine && isViewMode && (
          <div className="bg-white rounded-2xl border border-gray-200 p-4 sm:p-6 shadow-sm mb-6">
            <h2 className="font-bold text-gray-900 text-base sm:text-lg mb-4 pb-2 border-b-2 border-gray-100">
              Signatures
            </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="text"
                  className="w-full bg-slate-100 border border-gray-300 rounded-lg px-3 py-2 text-sm text-slate-700 font-medium cursor-not-allowed"
        ...
        {selectedMachine && isViewMode && (
          <div className="bg-white rounded-2xl border border-gray-200 p-4 sm:p-6 shadow-sm mb-6">
            ...
            <textarea
              ...
              className="w-full bg-slate-50 border border-gray-300 rounded-lg px-3 py-2 text-sm text-slate-700 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition disabled:bg-gray-100 disabled:text-gray-500 disabled:cursor-not-allowed"
            />
            ...
            {isAlreadyReviewed ? (
              ...
              <div className="flex flex-col sm:flex-row gap-3 mt-4">
                <button
                  ...
                  className="flex-1 bg-red-600 hover:bg-red-700 text-white py-3 rounded-xl font-bold transition disabled:opacity-70 disabled:cursor-not-allowed"
                >
                ...
                <button
                  ...
                  disabled={approvalLoading}
                  className="flex-1 bg-green-600 hover:bg-green-700 text-white py-3 rounded-xl font-bold transition disabled:opacity-70 disabled:cursor-not-allowed"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 items-end">
              ...
              <div>
                ...
                <input
                  ...
                  value={signatures.verifiedBy}
                  onChange={(e) =>
                    setSignatures({ ...signatures, verifiedBy: e.target.value })
                  }
                />
              </div>
              ...
              <div className="sm:text-right">
                ...
                >
                  {isSubmitting ? (
                    ...
                    <>
                      <i className="bi bi-cloud-arrow-up-fill mr-2"></i> Save
                      Checksheet
                    </>
                  )}
                </button>

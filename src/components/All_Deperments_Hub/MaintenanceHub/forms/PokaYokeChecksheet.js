...
const getLoggedInUsername = () => {
  ...
  const jsonKeys = ["user", "authUser", "currentUser", "userData", "profile"];
...
const initialChecks = [
  {
    id: 1,
    detailEng: "Air Pressure switch",
    detailHin: "एयर प्रेशर स्विच",
    method: "BY HAND OPERATED",
    status: "",
    remarks: "",
  },
  {
    id: 2,
    detailEng: "Photo Guard sensor",
    detailHin: "फोटो गार्ड सेंसर",
    method: "CHECKED BY HAND",
    status: "",
    remarks: "",
  },
  {
    id: 3,
    detailEng: "Grease sensor",
    detailHin: "ग्रीस सेंसर",
    method: "VISUAL CHECK",
    status: "",
    remarks: "",
  },
  {
    id: 4,
    detailEng: "Emergency Push Button",
    detailHin: "इमरजेंसी पुश बटन",
    method: "BY HAND OPERATED",
    status: "",
    remarks: "",
  },
  {
    id: 5,
    detailEng: "TDC limit switch",
    detailHin: "TDC लिमिट स्विच",
    method: "VISUAL CHECK",
    status: "",
    remarks: "",
  },
...
const buildChecklistFromBackend = (checkPoints = []) => {
  if (!Array.isArray(checkPoints) || checkPoints.length === 0) {
    return initialChecks.map((item) => ({ ...item }));
  }

  return initialChecks.map((item) => {
    const match = checkPoints.find((cp) => Number(cp.s_no) === Number(item.id));
    ...
    return {
      ...
      status:
        match.is_ok === true ||
        String(match.status || "").toUpperCase() === "OK"
          ? "OK"
          : "NG",
...
const PokaYokeChecksheet = () => {
  ...
  useEffect(() => {
    if (selectedPlant === "Plant 1") {
      const p1 = Array.from(
        { length: 57 },
        (_, i) => `PP-${String(i + 1).padStart(2, "0")}`
      );
      setMachineList(p1);
  ...
  useEffect(() => {
    ...
    if (selectedMachine && selectedPlant) {
      const isFilled = existingDataList.some(
        (data) => data.machine_no === selectedMachine
      );
      setShowWarning(isFilled);
    } else {
      setShowWarning(false);
    }
  }, [selectedMachine, existingDataList, selectedPlant, isViewMode]);
  ...
  useEffect(() => {
    ...
    fetchTodayData();
  }, [selectedPlant, selectedDate, isViewMode]);
    ...
    const fetchSingleReport = async () => {
      try {
        ...
        const res = await axios.get(
          `${API_BASE_URL}/api/get-single-maintenance-report/poka-yoke/${id}/`
        );

        if (!res.data.success) {
          alert(res.data.error || "Failed to load Poka Yoke Checksheet.");
          return;
        }

        const data = res.data.data || {};

        setSelectedDate(data.date || todayDate);
        setSelectedPlant(data.plant_name || "");
        setSelectedMachine(data.machine_no || "");

        setSignatures({
          checkedBy: data.checked_by_maintenance || "",
          verifiedBy: data.verified_by_production || "",
        });

        setChecklist(buildChecklistFromBackend(data.check_points || []));
        setApprovalRemark(data.approval_remarks || data.remarks || "");
      } catch (err) {
        console.error("Error loading Poka Yoke Checksheet:", err);
        alert("Failed to load Poka Yoke Checksheet.");
      } finally {
  ...
  const handleStatusChange = (checkId, newStatus) => {
    ...
    setChecklist((prev) =>
      prev.map((item) =>
        item.id === checkId ? { ...item, status: newStatus } : item
      )
    );
  };

  const handleRemarkChange = (checkId, text) => {
    if (isViewMode) return;

    setChecklist((prev) =>
      prev.map((item) =>
        item.id === checkId ? { ...item, remarks: text } : item
      )
    );
  };

  const openSopModal = (checkId) => {
    setCurrentSop(sopDatabase[checkId]);
  ...
  const handleSubmit = async (e) => {
    ...
    const alreadyExistsForThisMachine = existingDataList.some(
      (data) => data.machine_no === selectedMachine
    );

    if (alreadyExistsForThisMachine) {
      alert(
        `⚠️ Machine ${selectedMachine} का data आज के लिए पहले ही भरा जा चुका है!\n\nYou cannot submit again for this machine today.`
      );
      return;
    }
    ...
    try {
      ...
      if (response.ok && result.success !== false) {
        ...
        setExistingDataList((prev) => [
          {
            id: result.report_id || result.record_id || Date.now(),
            date: selectedDate,
            plant_name: selectedPlant,
            machine_no: selectedMachine,
            checked_by_maintenance: signatures.checkedBy,
  ...
  const handleApprove = async () => {
    try {
      ...
      const currentUser = getLoggedInUsername() || "Approver";

      await axios.post(`${API_BASE_URL}/api/approve-report/`, {
        log_id: id,
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      await axios.post(`${API_BASE_URL}/api/reject-report/`, {
        log_id: id,
        approver_username: currentUser,
        remarks: approvalRemark,
      });
            ...
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Machine: {selectedExistingData.machine_no} | Date:{" "}
                  {selectedExistingData.date}
                </p>
              </div>
            ...
            <div className="mb-6 p-4 bg-green-50 rounded-xl border-l-4 border-green-500">
              <div className="flex items-center gap-2">
                ...
                <span className="text-green-800 font-medium">
                  ✓ Data filled on {selectedExistingData.date}
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-gray-50 rounded-lg p-3">
                ...
                <div className="font-semibold text-gray-900">
                  {selectedExistingData.checked_by_maintenance || "Not provided"}
                </div>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-3">
                ...
                <div className="font-semibold text-gray-900">
                  {selectedExistingData.verified_by_production || "Not provided"}
                </div>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-['Inter',system-ui]">
          ...
          <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 rounded-t-2xl">
              <div className="flex justify-between items-center">
                <h4 className="text-xl font-bold text-gray-900">
                  {currentSop.titleEng}
                  <span className="text-emerald-600 text-base ml-2">
                    ({currentSop.titleHin})
                  </span>
                </h4>
            ...
            <div className="p-6">
              <div className="mb-6 p-4 bg-slate-50 rounded-xl border-l-4 border-emerald-500">
                <div className="font-bold text-emerald-600 mb-2 text-sm">
                  <i className="bi bi-info-circle-fill mr-2"></i>निर्देश
                  (Instructions):
                </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="border-2 border-emerald-500 rounded-xl p-4 bg-emerald-50">
                  <h5 className="text-emerald-700 font-bold text-center mb-3">
                    <i className="bi bi-check-circle-fill mr-2"></i> सही स्थिति
                    (OK Condition)
                  </h5>
                  <div className="bg-white rounded-lg h-48 flex items-center justify-center overflow-hidden">
                    <img
                      ...
                      onError={(e) =>
                        (e.target.src =
                          "https://placehold.co/400x250/d1fae5/059669?text=Image+Not+Found")
                      }
                    />
                  </div>
                </div>
                ...
                <div className="border-2 border-red-500 rounded-xl p-4 bg-red-50">
                  <h5 className="text-red-600 font-bold text-center mb-3">
                    <i className="bi bi-x-circle-fill mr-2"></i> गलत स्थिति (NG
                    Condition)
                  </h5>
                  <div className="bg-white rounded-lg h-48 flex items-center justify-center overflow-hidden">
                    <img
                      ...
                      onError={(e) =>
                        (e.target.src =
                          "https://placehold.co/400x250/fee2e2/e11d48?text=Image+Not+Found")
                      }
                    />
                  </div>
                </div>
            ...
            <div className="sticky bottom-0 bg-white border-t border-gray-100 px-6 py-4 rounded-b-2xl">
              <button
                type="button"
                onClick={() => setShowInfoModal(false)}
                className="px-6 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg font-semibold transition"
              >
                Close (बंद करें)
              </button>
            </div>
      ...
      <nav className="fixed top-0 left-0 right-0 bg-white border-b border-gray-200 z-[10000] shadow-sm">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14 sm:h-16">
            <div
              onClick={() =>
                navigate(isViewMode ? -1 : "/Maintenance/Machine/daily")
              }
      ...
      <div className="pt-20 sm:pt-24 pb-8 px-4 sm:px-6 lg:px-8 max-w-[1400px] mx-auto">
        {loadingReport ? (
          <div className="bg-white rounded-2xl border border-gray-200 p-8 text-center font-bold text-gray-500 shadow-sm">
          ...
          <>
            <div className="bg-white rounded-2xl border border-gray-200 p-4 sm:p-6 mb-6 shadow-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1 block">
                    Select Plant (प्लांट चुनें)
                  </label>
                  <select
                    className="w-full bg-slate-50 border border-gray-300 rounded-lg px-3 py-2 text-sm font-medium focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition text-slate-700 disabled:bg-slate-100 disabled:cursor-not-allowed"
                    value={selectedPlant}
                ...
                <div className="relative">
                  ...
                  <select
                    ...
                    disabled={!selectedPlant || isViewMode}
                  >
                    <option value="">Select Machine...</option>

                    {selectedMachine && !machineList.includes(selectedMachine) && (
                      <option value={selectedMachine}>{selectedMachine}</option>
                    )}

                    {machineList.map((mc, index) => {
                      const isFilled = existingDataList.some(
                        (data) => data.machine_no === mc
                      );
                  ...
                  {isLoadingData && (
                    <div className="absolute right-3 top-8">
                      <i className="bi bi-arrow-repeat animate-spin text-emerald-500"></i>
                    </div>
                  )}
                </div>
              </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-red-800 font-bold text-sm">
                      ⚠️ Machine Already Filled!
                    </h3>
                    <p className="text-red-700 text-sm">
                      Machine {selectedMachine} का data आज के लिए पहले ही भरा जा
                      चुका है!
                    </p>
                    <p className="text-red-600 text-xs mt-1">
                      This machine&apos;s checksheet has already been submitted
                      for today. You cannot submit again.
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-slate-50">
                      <tr>
                        <th className="px-3 sm:px-4 py-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">
                          Date
                        </th>
                        <th className="px-3 sm:px-4 py-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">
                          Machine No.
                        ...
                        <tr>
                          ...
                          >
                            {isLoadingData
                              ...
                              : "No checksheet filled today for this plant."}
                          </td>
                        </tr>
                      )}
                    </tbody>
            ...
            {(selectedMachine && !showWarning) || isViewMode ? (
              <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm mb-6">
                <div className="p-4 sm:p-6 pb-0">
                  <div className="flex items-center gap-2 mb-4 pb-2 border-b-2 border-gray-100">
                    <i className="bi bi-ui-checks-grid text-emerald-600 text-lg"></i>
                    <h2 className="font-bold text-gray-900 text-base sm:text-lg">
                      {isViewMode ? "View Checksheet Entry" : "New Checksheet Entry"}
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {checklist.map((item) => (
                        <tr key={item.id} className="hover:bg-slate-50 transition">
                          <td className="px-3 sm:px-4 py-3 text-center text-sm font-semibold text-gray-500">
                            {item.id}
                          </td>
                          <td className="px-3 sm:px-4 py-3">
                          ...
                          <td className="px-3 sm:px-4 py-3">
                            <input
                              type="text"
                              className="w-full bg-slate-50 border border-gray-300 rounded-lg px-2 py-1.5 text-xs text-slate-700 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-200 outline-none transition disabled:bg-slate-100 disabled:cursor-not-allowed"
            ...
            {(selectedMachine && !showWarning) || isViewMode ? (
              <div className="bg-white rounded-2xl border border-gray-200 p-4 sm:p-6 shadow-sm">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 items-end">
                  <div>
                    <label className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1 block">
                      Checked By (Maintenance Person)
                    </label>
                    <input
                      type="text"
                      className="w-full bg-slate-50 border border-gray-300 rounded-lg px-3 py-2 text-sm text-slate-700 font-medium focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition disabled:bg-slate-100 disabled:cursor-not-allowed"
                      placeholder="Name / Sign"
                      value={signatures.checkedBy}
                      disabled={isViewMode}
                      onChange={(e) =>
                        setSignatures({ ...signatures, checkedBy: e.target.value })
                      }
                    />
                  </div>
                  ...
                  <div>
                    <label className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1 block">
                      Verified By (Production Engineer)
                    </label>
                    <input
                      type="text"
                      className="w-full bg-slate-50 border border-gray-300 rounded-lg px-3 py-2 text-sm font-medium text-slate-700 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition disabled:bg-slate-100 disabled:cursor-not-allowed"
                      placeholder="Name / Sign"
                  ...
                  <div className="mt-6 border-t border-gray-200 pt-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                      <button
                        ...
                        className="w-full bg-red-500 hover:bg-red-600 text-white font-bold px-6 py-2.5 rounded-xl transition shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                      ...
                      <button
                        ...
                        disabled={approvalLoading}
                        className="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-bold px-6 py-2.5 rounded-xl transition shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                      >

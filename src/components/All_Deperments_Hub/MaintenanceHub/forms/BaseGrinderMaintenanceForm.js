...
const BaseGrinderMaintenanceForm = () => {
  ...
  const { id } = useParams();
  const isViewMode=Boolean(id); // <--- Check if the form is opened in view mode (from notification)
            ...
            const fetchReport = async () => {
              try {
                const res = await axios.get(
                  `${API_BASE_URL}/api/get-single-maintenance-report/base-grinder/${id}/`
                );
                ...
                if (res.data.success) {
                  ...
                  setMetaData({
                    machineName: meta.machineName || 'Base Grinder Machine', // Default value if not provided
                    date: meta.date || '',
                  ...
                  errorAlert(res.data.error || "Failed to load Base Grinder Check Sheet.");
                }
              } catch (err) {
                console.error("Error loading Check Sheet:", err);
                errorAlert(err.response?.data?.error || "Failed to load Base Grinder Check Sheet.");
              }
            };
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      warningAlert("Please fill all required fields in General Information and Prepared By.");
      return;
    }

...
const BaseGrinderMaintenanceForm = () => {
  ...
  const handleSubmit = (e) => {
    ...
    console.log("Base Grinder Form Submitted:", { id: Date.now(), metaData, checklist: tableData });
    setShowSuccess(true);
    setTimeout(() => {
    ...
    <div className="container-fluid py-4" style={{ backgroundColor: '#f8fafc', minHeight: '100vh' }}>
      <style>{`
        ...
        * { font-family: 'Inter', sans-serif; }
        ...
        /* Base Grinder Theme Color: Orange */
        .header-accent { background: #f97316; color: white; padding: 20px; border-radius: 20px 20px 0 0; }
        .btn-save { background: #f97316; border: none; padding: 12px 40px; border-radius: 50px; font-weight: 600; transition: 0.3s; }
        .btn-save:hover { background: #ea580c; box-shadow: 0 5px 15px rgba(249, 115, 22, 0.4); }
        .method-badge { background: #fff7ed; color: #ea580c; padding: 4px 12px; border-radius: 6px; font-weight: 600; font-size: 0.8rem; }
      ...
      {showSuccess && <div className="position-fixed bottom-0 end-0 m-4 bg-dark text-white px-4 py-3 rounded-4 shadow-lg">✨ Base Grinder record saved successfully!</div>}
    </div>
  );
};

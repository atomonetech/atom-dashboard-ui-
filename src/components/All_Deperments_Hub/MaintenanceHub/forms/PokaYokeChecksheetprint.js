...
// A4 Print Ready — Browser Portrait/Landscape Auto Layout
// ============================================================
...
const PokaYokeChecksheetprint = ({ onEditForm }) => {
  ...
  const [filterYear] = useState(currentYearStr);
  ...
  const totalRows = Math.max(reportData.length, 1);

  // ── FETCH MACHINES FROM BACKEND WHEN PLANT CHANGES ──
  useEffect(() => {
    const fetchMachines = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/api/machines/list/?plant=${encodeURIComponent(apiPlantNameForList)}`
        );

        const result = await response.json();

        if (result.success && result.machines) {
          const formattedMachines = result.machines.map((m) =>
            typeof m === 'number' ? `PP-${String(m).padStart(2, '0')}` : m
          );
  ...
  // ── FETCH MONITORING DATA FROM BACKEND WHEN APPLY IS CLICKED ──
  const handleApplyFilter = async () => {
    ...
    try {
      ...
      const url =
        `${API_BASE_URL}/api/get_monitoring_data/` +
        `?machine_no=${encodeURIComponent(filterMachine)}` +
        `&plant_name=${encodeURIComponent(filterPlant)}` +
        `&month=${monthIndex}` +
        `&year=${filterYear}`;
      ...
      const newItems = JSON.parse(JSON.stringify(DEFAULT_ITEMS));
  ...
  const titleColSpan = Math.max(1, daysInMonth - 5);

  const dateHeaderSpan1 = Math.floor(daysInMonth / 4);
  const dateHeaderSpan2 = Math.floor(daysInMonth / 4);
  const dateHeaderSpan3 = Math.floor(daysInMonth / 4);
  const dateHeaderSpan4 = daysInMonth - (dateHeaderSpan1 + dateHeaderSpan2 + dateHeaderSpan3);
  ...
  const TH =
    'border border-black font-bold text-center align-middle leading-tight bg-[#f5f5f5] text-black px-0.5 py-0.5 text-[9px] break-words uppercase';

  const TD =
    'border border-black text-center align-middle bg-white px-1 py-0.5 text-black text-[11px]';

  const InfoLabel =
    'info-label border border-black font-bold bg-[#c0c0c0] text-black text-[11px] px-2 py-1 text-left whitespace-nowrap uppercase';

  const InfoValue =
    'info-value border border-black font-semibold bg-white text-black text-[11px] px-2 py-1 text-center whitespace-nowrap uppercase';
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f5] p-4 text-black print:p-0 print:m-0 print:bg-white print:min-h-0">
      {/* ── Top Bar ── */}
            ...
            <div className="absolute right-0 mt-2 w-72 bg-white rounded-lg shadow-xl border border-gray-200 z-50">
              ...
              <div className="p-4 flex flex-col gap-4">
                ...
                <div className="relative">
                  ...
                  >
                    <option value="">
                      {isLoadingMachines ? 'Loading...' : 'Select Machine...'}
                    </option>
              ...
              <div className="flex gap-2 p-3 border-t bg-gray-50 rounded-b-lg">
                <button
                  ...
                  className="flex-1 bg-[#4285F4] hover:bg-[#3367D6] text-white py-2 rounded text-sm font-bold flex justify-center items-center gap-2 border-none cursor-pointer disabled:opacity-50"
                >
                  {isLoadingData ? (
      ...
      <style>{`
        ...
        /* ✅ Screen preview columns - collapse fix */
        .col-sr {
          width: 2.2%;
        }
        ...
        /* Browser print dialog me Portrait select hoga to ye layout apply hoga */
        @media print and (orientation: portrait) {
          ...
          .print-table th,
          .print-table td {
            padding: 0.35mm 0.25mm !important;
          ...
          .data-row {
            height: calc(215mm / var(--body-row-count)) !important;
          }
        ...
        /* Browser print dialog me Landscape select hoga to ye layout apply hoga */
        @media print and (orientation: landscape) {
          ...
          .data-row {
            height: calc(110mm / var(--body-row-count)) !important;
          }
      ...
      >
        <table
          className="print-table border border-black"
          style={{
            "--day-count": daysInMonth,
            "--body-row-count": totalRows,
          }}
        >
          <colgroup>
            <col className="col-sr" />
            <col className="col-mc" />
          ...
          <thead className="table-header-group">
            <tr className="header-main-row h-[28px]">
              <th
                colSpan={3}
                rowSpan={3}
              ...
              >
                <img
                  src={atomone}
                  alt="ATOM ONE"
                  className="print-logo max-h-[70px] max-w-full block mx-auto object-contain"
                />
              </th>
              ...
              >
                <h1 className="print-title text-[26px] font-bold uppercase tracking-[1px] m-0 text-black">
                  POKA YOKE MONITORING SHEET
                </h1>
              </th>

              <th colSpan={4} className={InfoLabel}>
                DOC NO.
              </th>

              <th colSpan={3} className={InfoValue}>
                AOT-F-QC-07A
              </th>
            </tr>

            <tr className="header-small-row h-[28px]">
              <th colSpan={4} className={InfoLabel}>
                REVISION NO.
              </th>
            ...
            <tr className="date-label-row h-[24px]">
              ...
              <th colSpan={dateHeaderSpan1} className={`${TH} fixed-text-cell`}>
                DATE
              </th>

              <th colSpan={dateHeaderSpan2} className={`${TH} fixed-text-cell`}></th>

              <th
                colSpan={dateHeaderSpan3}
                className={`${TH} fixed-text-cell text-[#3b5998]`}
              >
                MONTH: {appliedMonth}
              </th>

              <th colSpan={dateHeaderSpan4} className={`${TH} fixed-text-cell`}></th>
            </tr>

            <tr className="date-number-row h-[20px]">
              {Array.from({ length: daysInMonth }).map((_, i) => (
                <th key={i} className={`${TH} day-header`} style={{ fontSize: '8px' }}>
                  {i + 1}
                </th>
              ))}

              <th className={`${TH} remarks-cell`} style={{ fontSize: '8px' }}>
                Remarks
              </th>
            </tr>
          </thead>
          ...
          <tbody>
            {reportData.map((row, i) => (
              <tr key={i} className="data-row break-inside-avoid">
                <td className={`${TD} fixed-text-cell font-bold`}>
                  {row.sr_no || i + 1}
                </td>

                <td
                  className="mc-cell bg-white text-center align-middle text-[12px] font-bold text-black"
                  style={mcCellStyle(i)}
                >
                  {i === 2 ? appliedMachine : ''}
                </td>

                <td className={`${TD} fixed-text-cell text-left pl-2 font-semibold`}>
                  {row.detail || ''}
                </td>

                <td className={`${TD} method-cell`}>
                  {row.method || ''}
                </td>
                ...
                {Array.from({ length: daysInMonth }).map((_, d) => {
                  ...
                  return (
                    <td key={d} className={`${TD} day-cell ${textColor}`}>
                      {cellValue}
                    </td>
                  );
                })}

                <td className={`${TD} remarks-cell text-left pl-1`}>
                  {row.remarks || ''}
                </td>
              </tr>
            ))}
          ...
          <tfoot className="table-footer-group">
            <tr className="checked-row h-[26px]">
              <td
                colSpan={12}
                className="footer-text border border-black bg-white px-3 py-1 text-[11px] text-left"
              >
            ...
            <tr className="footer-title-row h-[22px]">
              <td
                colSpan={4}
                rowSpan={4}
                className="footer-text border border-black p-2 text-[10px] font-bold align-bottom text-center bg-white"
              >
                PREPARED BY
              </td>
              ...
              <td
                colSpan={legendsSpan}
                className="footer-text border border-black p-1 text-[11px] font-bold text-center bg-[#f0f0f0]"
              >
                LEGENDS
              </td>

              <td
                colSpan={approvedSpan}
                rowSpan={4}
                className="footer-text border border-black p-2 text-[10px] font-bold align-bottom text-center bg-white"
              >
                APPROVED BY
            ...
            <tr className="footer-legend-row h-[20px]">
              <td
                colSpan={iconSpan}
                className="legend-symbol border border-black p-0.5 text-center font-bold text-[14px] bg-white text-green-600"
              >
                ✓
              </td>
              ...
              <td
                colSpan={textSpan}
                className="footer-text border border-black p-0.5 text-center text-[10px] bg-white"
              >
                OK
              </td>
            </tr>
            ...
            <tr className="footer-legend-row h-[20px]">
              <td
                colSpan={iconSpan}
                className="legend-symbol border border-black p-0.5 text-center font-bold text-[14px] bg-white text-red-600"
              >
                X
              </td>
            ...
            <tr className="footer-legend-row h-[20px]">
              <td
                colSpan={iconSpan}
                className="legend-symbol border border-black p-0.5 text-center font-bold text-[10px] bg-white"
              >
                N/A
              </td>

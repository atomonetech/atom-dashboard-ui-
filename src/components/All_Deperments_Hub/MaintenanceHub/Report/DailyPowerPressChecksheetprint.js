...
//  Daily Power Press Check Sheet — Multi-Page Ready (A3 Landscape)
//  Fixed: Auto-stretch rows to fill empty bottom space & Reset logic applied
// ============================================================
...
// Month names array added for global access
...
const getDaysInMonth = (monthName, year) => {
  if (!monthName || !year) return 31; // Blank hone par by default 31 days
...
// Removed hardcoded default props for month, replaced with dynamic fetching
const DailyPowerPressChecksheetprint = ({ items = DEFAULT_ITEMS, onEditForm }) => {
  ...
  // Current Date Setup for default filter values
  ...
  // ✅ Table ke liye Initially Blank State
  ...
  const handleResetFilter = () => {
    // Reset karne par wapas filter today par set ho jayega, par table blank ho jayegi
    setFilterMonth(currentMonthName);
    setFilterMachine("PP-01");
    setAppliedMonth(""); // Blank
    setAppliedMachine(""); // Blank
    setIsFilterOpen(false);
  };

  // ✅ Agar appliedMonth blank hai toh default 31 days banenge aur backend data show nahi hoga
  ...
  const TH = 'border border-black font-bold text-center align-middle leading-tight bg-[#f5f5f5] text-black px-1 py-1 text-[10px] break-words uppercase';
  const TD = 'border border-black text-center align-middle bg-white px-1 py-1 text-black text-[10px] uppercase overflow-hidden';
  const InfoLabel = 'border border-black font-bold bg-[#f5f5f5] text-black text-[10px] px-2 py-1 text-left uppercase whitespace-nowrap overflow-hidden text-ellipsis';
  const InfoValue = 'border border-black font-semibold bg-white text-black text-[10px] px-2 py-1 text-center whitespace-nowrap overflow-hidden text-ellipsis';
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f5] p-4 text-black overflow-auto">
      ...
      <div className="flex justify-end items-center gap-3 mb-4 print:hidden">
        <button onClick={() => navigate("/maintenance-hub")} className="bg-[#607d8b] hover:bg-[#4d646f] text-white border-none px-4 py-2 rounded-md font-bold cursor-pointer text-sm flex items-center gap-1.5 shadow-sm">
          <i className="bi bi-arrow-left-circle-fill"></i> Back
        </button>

        <div className="relative" ref={filterRef}>
          <button onClick={() => setIsFilterOpen(!isFilterOpen)} className="bg-[#3b5998] hover:bg-[#2d4373] text-white border-none px-4 py-2 rounded-md font-bold cursor-pointer text-sm flex items-center gap-1.5 shadow-sm">
            ...
            <div className="absolute right-0 mt-2 w-72 bg-white rounded-lg shadow-xl border border-gray-200 z-50">
              ...
              <div className="p-4 flex flex-col gap-4">
                <div>
                  ...
                  <select value={filterMonth} onChange={(e) => setFilterMonth(e.target.value)} className="w-full border border-gray-300 rounded p-2 text-sm focus:outline-none focus:border-[#3b5998]">
                    {MONTH_NAMES.map(m => (
                ...
                <div>
                  ...
                  <select value={filterMachine} onChange={(e) => setFilterMachine(e.target.value)} className="w-full border border-gray-300 rounded p-2 text-sm focus:outline-none focus:border-[#3b5998]">
              ...
              <div className="flex gap-2 p-3 border-t bg-gray-50 rounded-b-lg">
                <button onClick={handleApplyFilter} className="flex-1 bg-[#4285F4] hover:bg-[#3367D6] text-white py-2 rounded text-sm font-bold flex justify-center items-center gap-2 border-none cursor-pointer">
        ...
        {onEditForm && (
          <button onClick={() => onEditForm()} className="bg-[#ff9800] hover:bg-[#e68a00] text-white border-none px-4 py-2 rounded-md font-bold cursor-pointer text-sm flex items-center gap-1.5 shadow-sm">
            <i className="bi bi-pencil-square"></i> Edit
          </button>
        )}

        <button onClick={() => window.print()} className="bg-[#4CAF50] hover:bg-[#43a047] text-white border-none px-4 py-2 rounded-md font-bold cursor-pointer text-sm flex items-center gap-1.5 shadow-sm">
      ...
      {/* ── Main Container: h-[297mm] forces exact A3 height ── */}
      <div className="bg-white mx-auto shadow-lg font-sans w-[420mm] h-[297mm] box-border p-[4mm] print:w-full print:h-[100vh] print:p-0 print:m-0 print:shadow-none flex flex-col overflow-hidden">
        ...
        <table className="w-full h-full border-collapse table-fixed border border-black">
          <colgroup>
            <col className="w-[3.0%]" />  
            <col className="w-[12.0%]" /> 
            <col className="w-[18.0%]" /> 
            <col className="w-[9.0%]" />  
            
            {Array.from({ length: safeDisplayedDays }).map((_, i) => (
              <col key={i} style={{ width: `${58.0 / safeDisplayedDays}%` }} />
            ))}
          </colgroup>
          ...
          <thead className="table-header-group">
            <tr className="h-[28px]">
              ...
              <th colSpan={titleColSpan} rowSpan={3} className="border border-black text-center align-middle bg-white">
                <h1 className="text-[28px] font-bold uppercase tracking-[1.5px] m-0 text-black whitespace-nowrap overflow-hidden text-ellipsis px-2">
                  DAILY POWER PRESS CHECK SHEET
                </h1>
              </th>
              <th colSpan={topLabelSpan} className={InfoLabel}>DOC NO.</th>
              <th colSpan={topValueSpan} className={InfoValue}>AOT-F-MM-02</th>
            </tr>

            <tr className="h-[28px]">
              <th colSpan={topLabelSpan} className={InfoLabel}>REVISION</th>
              <th colSpan={topValueSpan} className={InfoValue}>03</th>
            </tr>

            <tr className="h-[28px]">
              <th colSpan={topLabelSpan} className={InfoLabel}>DATE</th>
              <th colSpan={topValueSpan} className={InfoValue}>31.05.2024</th>
            </tr>

            <tr className="h-[24px]">
              <th rowSpan={2} className={TH}>S. NO.</th>
              <th rowSpan={2} className={TH}>CHECK POINT</th>
              <th rowSpan={2} className={TH}>SPECIFICATION</th>
              <th rowSpan={2} className={TH}>CHECKING METHOD</th>
              
              <th colSpan={span1} className={`${TH} text-left pl-2 bg-white`}>DATE</th>
              <th colSpan={span2} className={`${TH} text-left pl-2 bg-white`}>MONTH: {appliedMonth}</th>
              <th colSpan={span3} className={`${TH} text-left pl-2 bg-white`}>TON:</th>
              <th colSpan={span4} className={`${TH} text-left pl-2 bg-white`}>PP: {appliedMachine}</th>
            </tr>

            <tr className="h-[20px]">
              {Array.from({ length: safeDisplayedDays }).map((_, i) => (
                <th key={i} className={TH} style={{ fontSize: '10px', padding: '1px 0', backgroundColor: 'white' }}>{i + 1}</th>
              ))}
            </tr>
          </thead>
          ...
          <tbody>
            {rowData.map((row, i) => (
              <tr key={i} className="break-inside-avoid">
                {!row.isSubRow && (
                  <>
                    <td className={`${TD} font-bold`} rowSpan={row.rowSpan || 1}>{row.sr_no}</td>
                    <td className={`${TD} font-bold`} rowSpan={row.rowSpan || 1}>{row.check_point}</td>
                  </>
                )}
                <td className={`${TD} font-semibold`}>{row.specification || ''}</td>
                <td className={TD}>{row.method || ''}</td>

                {Array.from({ length: safeDisplayedDays }).map((_, d) => (
                  <td key={d} className={TD}>
                    {/* ✅ Agar filter blank hai, toh koi purana save kiya hua data bhi nahi dikhega */}
          ...
          <tfoot className="table-footer-group">
            <tr className="h-[40px]">
              <td colSpan={4} className="border border-black bg-white px-3 py-1 text-[11px] text-center font-bold">
                MAINTENANCE INCHARGE SIGN.
              </td>
              {Array.from({ length: safeDisplayedDays }).map((_, d) => (
                <td key={`maint-${d}`} className={TD}></td>
              ))}
            </tr>
            ...
            <tr className="h-[40px]">
              ...
              <td colSpan={Math.floor(safeDisplayedDays / 3)} className={TD}></td>
              <td colSpan={safeDisplayedDays - (2 * Math.floor(safeDisplayedDays / 3))} className={TD}></td>
            </tr>

            <tr className="h-[25px]">
              <td colSpan={4} rowSpan={2} className="border border-black p-2 text-[12px] font-bold text-center bg-white align-middle">
                LEGENDS
              </td>
              <td colSpan={safeDisplayedDays - 4} rowSpan={2} className="border border-black p-1 text-[12px] font-bold text-center bg-white align-middle">
                CHECK POINT
              </td>
              <td colSpan={2} className="border border-black p-1 text-center font-bold text-[16px] bg-white align-middle">✓</td>
              <td colSpan={2} className="border border-black p-1 text-center text-[12px] bg-white font-bold align-middle">OK</td>
            </tr>
            <tr className="h-[25px]">
              <td colSpan={2} className="border border-black p-1 text-center font-bold text-[16px] bg-white align-middle">X</td>
              <td colSpan={2} className="border border-black p-1 text-center text-[12px] bg-white font-bold align-middle">NOT OK</td>
            </tr>
          </tfoot>

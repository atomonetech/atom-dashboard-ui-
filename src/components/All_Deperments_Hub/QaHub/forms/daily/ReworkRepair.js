...
const SelectField = ({ value, onChange, options, placeholder, disabled }) => (
  <div style={{ position: "relative", width: "100%" }}>
    <select value={value} onChange={onChange} disabled={disabled}
      style={{ ...selectBase, color: value ? "#111827" : "#9ca3af", opacity: disabled ? 0.6 : 1, cursor: disabled ? "not-allowed" : "pointer" }}>
      <option value="">{placeholder || "Select..."}</option>
      {options.map(o => <option key={o} value={o}>{o}</option>)}
    </select>
    {!disabled && <IoIosArrowDown style={{ position: "absolute", right: 7, top: "50%", transform: "translateY(-50%)", color: "#6b7280", fontSize: 12, pointerEvents: "none" }} />}
  </div>
);
...
export default function ReworkRepair() {
  ...
  // Fetch Dropdowns & Handle View Mode Data
  useEffect(() => {
    ...
    if (isViewMode) {
      const fetchReportData = async () => {
        try {
          const res = await axios.get(`${API_BASE_URL}/api/get-single-report/rework-view/${id}/`);
          if (res.data.success) {
            ...
            // Backend return status as " OK" or "NOT OK", convert back
            let st = null;
            if(data.Status && data.Status.trim() === 'OK') st = 'ok';
            if(data.Status && data.Status.trim() === 'NOT OK') st = 'notok';
            ...
            setRows([{
              ...
              okStatus: st,
              observations: obsArr,
              inspectedBy: data['Inspected By'] === '—' ? "" : data['Inspected By'],
            }]);
  ...
  const handleQtyKeyDown = (e) => { if (!isViewMode && e.key === "Enter") handleAddRow(); };
  ...
  const addObservation = (i) => {
     if(isViewMode) return;
     setRows(prev => prev.map((r, idx) => idx === i ? { ...r, observations: [...r.observations, ""] } : r ));
  }

  const toggleStatus = (i, type) => {
     if(isViewMode) return;
     setRows(prev => prev.map((r, idx) => idx === i ? { ...r, okStatus: r.okStatus === type ? null : type } : r));
  }

  const removeRow = i => {
     if(!isViewMode) setRows(prev => prev.filter((_, idx) => idx !== i));
  }

  // 🔥 UPDATED SUBMIT LOGIC (Approve & Save)
  ...
  // Helper function to disable inputs visually
  ...
  const obsInputStyle = {
    ...
    borderRadius: 5, textAlign: "center", fontSize: 12, color: isViewMode ? "#6b7280" : "#111827",
    background: isViewMode ? "#f8fafc" : "#fff", fontFamily: "'Inter',sans-serif", outline: "none", boxSizing: "border-box", display: "block"
  };
    ...
    <div style={{ minHeight: "100vh", background: "#f0f4f8", fontFamily: "'Inter',sans-serif", padding: isMobile ? "10px 6px" : "20px 16px", display: "flex", flexDirection: "column", alignItems: "center" }}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .save-btn { height: 38px; padding: 0 22px; border-radius: 8px; background: ${isViewMode ? '#10b981' : '#2563eb'}; color: #fff; border: none; font-weight: 700; font-size: 13px; cursor: pointer; font-family: 'Inter', sans-serif; display: flex; align-items: center; gap: 6px; transition: background 0.15s, transform 0.12s, box-shadow 0.15s; box-shadow: 0 2px 8px rgba(37,99,235,0.18); white-space: nowrap; }
        .save-btn:hover { filter: brightness(0.9); box-shadow: 0 4px 14px rgba(37,99,235,0.28); transform: translateY(-1px); }
        .save-btn:active { filter: brightness(0.8); transform: scale(0.97); box-shadow: none; }
      ...
      <div style={{ width: "100%", maxWidth: 1400, background: "#fff", borderRadius: 12, boxShadow: "0 1px 4px rgba(0,0,0,0.07)", border: "1px solid #e2e8f0", marginBottom: 14, overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10, padding: isMobile ? "12px" : "12px 22px", borderBottom: "1px solid #e2e8f0" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <button onClick={() => navigate(-1)} style={{ width: 34, height: 34, borderRadius: 8, background: "#f1f5f9", border: "1px solid #e2e8f0", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
            ...
            <input value={date} onChange={e => setDate(e.target.value)} disabled={isViewMode} style={{ ...fieldBase, width: 118, fontWeight: 600, background: isViewMode ? "#f1f5f9" : "#fff", cursor: isViewMode ? "not-allowed" : "text" }} />
          </div>
        </div>

        {/* Quick-add form (Hide in View Mode if you want, but for now we'll just disable it) */}
        {!isViewMode && (
            ...
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "2.2fr 1.4fr 1.4fr 2fr 1fr", gap: 14, alignItems: "end" }}>
              ...
              <div>
                ...
                <input 
                  ...
                  style={{ ...getFldStyle(card.reworkQty), textAlign: "center", fontWeight: card.reworkQty !== "" ? 600 : 400 }} 
                  disabled={isViewMode}
                />
              </div>
            </div>
        ...
        <div style={{ width: "100%", maxWidth: 1400, background: "#fff", borderRadius: 12, boxShadow: "0 1px 4px rgba(0,0,0,0.07)", border: "1px solid #e2e8f0", overflow: "hidden" }}>
          <div style={{ padding: isMobile ? "12px" : "16px 20px" }}>
                ...
                <table className="rework-table">
                  ...
                  <thead>
                    ...
                    <tr>
                      ...
                      {!isViewMode && <th style={{ padding: "7px 2px", background: "#f0f7ff", border: "1px solid #d1d5db" }}></th>}
                    </tr>
                  </thead>
                  <tbody>
                    {rows.map((row, i) => {
                      ...
                      return (
                        <tr key={row.id} className="new-row" style={{ background: i % 2 === 0 ? "#ffffff" : "#f8fafc" }}>
                          ...
                          <TD style={{ background: isOk ? "#f0fdf4" : "inherit", textAlign: "center" }}>
                            <button className={isViewMode ? "" : "sbtn"} onClick={() => toggleStatus(i, "ok")} disabled={isViewMode} style={{ width: 36, height: 36, borderRadius: 8, border: isOk ? "2px solid #16a34a" : "1.5px solid #d1fae5", background: isOk ? "#dcfce7" : "#fff", color: isOk ? "#16a34a" : "#bbf7d0", fontSize: 18, fontWeight: 800, cursor: isViewMode ? "default" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto", boxShadow: isOk && !isViewMode ? "0 2px 8px rgba(22,163,74,0.18)" : "none" }}>✓</button>
                          </TD>
                          <TD style={{ background: isNotOk ? "#fff5f5" : "inherit", textAlign: "center" }}>
                            <button className={isViewMode ? "" : "sbtn"} onClick={() => toggleStatus(i, "notok")} disabled={isViewMode} style={{ width: 36, height: 36, borderRadius: 8, border: isNotOk ? "2px solid #dc2626" : "1.5px solid #fecaca", background: isNotOk ? "#fee2e2" : "#fff", color: isNotOk ? "#dc2626" : "#fecaca", fontSize: 14, fontWeight: 800, cursor: isViewMode ? "default" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto", boxShadow: isNotOk && !isViewMode ? "0 2px 8px rgba(220,38,38,0.18)" : "none" }}>✕</button>
                          </TD>
                          ...
                          <TD>
                            <input value={row.inspectedBy || ""} onChange={e => updateRow(i, "inspectedBy", e.target.value)} disabled={isViewMode} placeholder="Name..." style={{ ...fieldBase, fontSize: 11, padding: "5px 6px", background: isViewMode ? "#f8fafc" : "#fff", color: isViewMode ? "#6b7280" : "#111827", cursor: isViewMode ? "not-allowed" : "text" }} />
                          </TD>
                          {!isViewMode && (
            ...
            <div style={{ marginTop: 16, padding: "14px 16px", background: "#f8faff", borderRadius: 8, border: "1px solid #e2e8f0" }}>
              ...
              <textarea value={remark} onChange={e => setRemark(e.target.value)} disabled={isViewMode} placeholder="Write remark here..." rows={3}
                style={{ width: "100%", padding: "9px 12px", border: "1px solid #d1d5db", borderRadius: 6, fontSize: 13, color: isViewMode ? "#6b7280" : "#111827", background: isViewMode ? "#f8fafc" : "#fff", fontFamily: "'Inter',sans-serif", boxSizing: "border-box", outline: "none", resize: "vertical", lineHeight: 1.6, cursor: isViewMode ? "not-allowed" : "text" }} />
            </div>

            {/* ── Bottom Actions (Save & Reset OR Approve) ── */}
            <div style={{ marginTop: 16, display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap", alignItems: "center", borderTop: "1px solid #e2e8f0", paddingTop: 16 }}>
              
              {/* Prepared By Section (Visible in View Mode) */}
              <div style={{width: '100%', '@media (min-width: 640px)': {width: 'auto'}}}>
                {isViewMode && (
                   <div>
                     <p style={{fontSize: 11, fontWeight: 700, color: '#6b7280', textTransform: 'uppercase', marginBottom: 2}}>Prepared By</p>
                     <p style={{fontSize: 16, fontWeight: 900, color: '#2563eb', textTransform: 'capitalize', margin: 0}}>{preparedBy}</p>
                   </div>
                )}
              </div>

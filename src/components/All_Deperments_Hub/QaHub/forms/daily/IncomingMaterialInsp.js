...
function rkStyle(v) {
  return {
    width: "100%", padding: "9px 28px 9px 12px", border: "1.5px solid #cbd5e1", borderRadius: 6,
    fontSize: 12, background: v === "OK" ? "#ecfdf5" : v === "NOT OK" ? "#fef2f2" : "#ffffff",
    color: v === "OK" ? "#059669" : v === "NOT OK" ? "#dc2626" : "#64748b", fontWeight: v ? 700 : 400,
...
function obsStyle(v) {
  return {
    width: "100%", padding: "9px 4px", border: `1px solid ${v ? "#3b82f6" : "#cbd5e1"}`,
    borderRadius: 6, fontSize: 12, textAlign: "center", background: v ? "#eff6ff" : "#ffffff", color: "#0f172a", fontWeight: v ? 600 : 400,
    outline: "none", fontFamily: "'DM Sans',sans-serif", boxSizing: "border-box", transition: "all 0.2s ease"
  };
}
...
export default function IncomingMaterialInsp() {
  ...
  const headerFilled = !!(supplier && customer && selectedPartId && qty);
    ...
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "'DM Sans',sans-serif", padding: isMobile ? "12px 8px" : "32px 16px", display: "flex", flexDirection: "column", alignItems: "center" }}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .imi-save-btn { height: 42px; padding: 0 24px; border-radius: 8px; background: #2563eb; color: #fff; border: none; font-weight: 700; font-size: 14px; cursor: pointer; display: flex; align-items: center; gap: 8px; transition: all 0.2s ease; box-shadow: 0 4px 12px rgba(37,99,235,0.2); }
        .imi-save-btn:hover { background: #1d4ed8; transform: translateY(-1px); box-shadow: 0 6px 16px rgba(37,99,235,0.3); }
        .imi-save-btn:active { transform: translateY(0); box-shadow: 0 2px 8px rgba(37,99,235,0.2); }
      ...
      <div style={{ width: "100%", maxWidth: 1140, background: "#ffffff", borderRadius: 12, boxShadow: "0 4px 20px rgba(0,0,0,0.04)", border: "1px solid #e2e8f0", overflow: "hidden", marginBottom: 20 }}>
        <div style={{ display: "flex", alignItems: isMobile ? "flex-start" : "center", flexDirection: isMobile ? "column" : "row", justifyContent: "space-between", gap: 12, padding: isMobile ? "16px" : "16px 24px", borderBottom: "1px solid #e2e8f0", background: "#f8fafc" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <button onClick={() => window.history.back()} style={{ width: 36, height: 36, borderRadius: 8, background: "#e2e8f0", border: "none", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, cursor: "pointer", color: "#475569", transition: "all 0.2s" }} onMouseOver={e=>e.currentTarget.style.background="#cbd5e1"} onMouseOut={e=>e.currentTarget.style.background="#e2e8f0"}>
            ...
            <div>
              <span style={{ fontWeight: 800, fontSize: isMobile ? 14 : 16, color: "#0f172a", display: "block", lineHeight: 1.2 }}>Incoming Material Inspection Report</span>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, alignSelf: isMobile ? "flex-end" : "center" }}>
            ...
            <input type="date" value={date.split('-').reverse().join('-')} onChange={e => {
                const [y,m,d] = e.target.value.split('-');
                setDate(`${d}-${m}-${y}`);
            }} style={{ padding: "8px 12px", border: "1.5px solid #cbd5e1", borderRadius: 6, fontSize: 13, fontWeight: 600, color: "#0f172a", outline: "none", fontFamily: "'DM Sans',sans-serif", width: 135 }} />
          </div>
        </div>
        ...
        <div style={{ padding: isMobile ? "8px 16px 20px" : "12px 24px 24px" }}>
          <div style={grid6}>
            ...
            <div>
              ...
              <input value={coilNo} onChange={e => setCoilNo(e.target.value)} placeholder="Coil number" style={fldStyle(coilNo)} />
            </div>
            <div>
              <label style={LBL}>Invoice / Challan</label>
              <input value={invoiceNo} onChange={e => setInvoiceNo(e.target.value)} placeholder="Invoice number" style={fldStyle(invoiceNo)} />
            </div>
            <div>
              <label style={LBL}>QTY.</label>
              <input type="number" min="1" value={qty} onChange={e => setQty(e.target.value)} placeholder="Total Qty" style={fldStyle(qty)} />
            </div>
          </div>
        </div>

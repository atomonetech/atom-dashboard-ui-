...
export default function RedbinAttendance() {
  ...
  // View mode mein UI pe current date dikhane ki jagah DB date dikhayenge
  const [displayDate, setDisplayDate] = useState(formattedDate);
  const [displayMonthYear, setDisplayMonthYear] = useState(`${currentMonth} ${currentYear}`);
  ...
  useEffect(() => {
    if (isViewMode) {
      const fetchReportData = async () => {
        try {
          const res = await axios.get(`${API_BASE}/api/get-single-report/redbin-attendance-view/${id}/`);
          if (res.data.success) {
            const data = res.data.data;
            
            // Format Display Date & Month
            if (data.date) {
               const parts = data.date.split('-');
               if(parts.length === 3) setDisplayDate(`${parts[2]}-${parts[1]}-${parts[0]}`);
            }
            if (data.month && data.year) {
               setDisplayMonthYear(`${data.month} ${data.year}`);
            }

            // Map data to single row for table view
            setRows([{
              name: data.employee_name || "—",
              designation: data.designation || "—",
              attendance: { [data.date ? parseInt(data.date.split('-')[2]) : today.getDate()]: data.status || "" }
            }]);
  ...
  const removeRow = (i) => {
     if (!isViewMode) setRows((p) => p.filter((_, idx) => idx !== i));
  }
  ...
  const attSel = (val, disabled = false) => ({
    ...
    fontSize: 12,
    fontWeight: 700,
    background: disabled ? "#f8fafc" : val === "P" || val === "Present" ? "#fffbeb" : val === "A" || val === "Absent" ? "#fee2e2" : "#fff",
    color: disabled ? "#64748b" : "#000000",
    outline: "none",
    cursor: disabled ? "not-allowed" : "pointer",
    fontFamily: "'DM Sans',sans-serif",
    ...
    >
      <style>{`
        ...
        .rb-save-btn {
          ...
          background: ${isViewMode ? '#10b981' : '#d97706'}; color: #fff; border: none;
        ...
        .rb-save-btn:hover { filter: brightness(0.9); transform: translateY(-1px); }
        .rb-save-btn:active { transform: scale(0.97); box-shadow: none; }
      ...
      >
        ...
        <div className="redbin-topbar">
          <div className="redbin-topbar-left">
            ...
            <div>
              <span className="redbin-title">Red Bin Attendance Sheet {isViewMode && <span style={{color:'#f59e0b'}}>(REVIEW)</span>}</span>
          ...
          >
            ...
            <div className="redbin-table-wrap">
              ...
              >
                <thead>
                  <tr style={{ background: "#92400e" }}>
                    ...
                    <th style={{ padding: "8px 10px", color: "#fff", fontWeight: 700, fontSize: 11, textAlign: "center", borderRight: "1px solid rgba(255,255,255,0.15)", minWidth: 50, background: "#b45309", }}>
                      {isViewMode ? displayDate.split('-')[0] : today.getDate()} {displayMonthYear.slice(0, 3)}
                    </th>
                    {!isViewMode && (
                ...
                <tbody>
                  {rows.map((row, i) => {
                    const day = isViewMode && displayDate ? parseInt(displayDate.split('-')[0]) : today.getDate();
                    ...
                    return (
                      ...
                      >
                        <td style={{ padding: "8px 4px", borderBottom: "1px solid #fef3c7", borderRight: "1px solid #fef3c7", textAlign: "center", fontWeight: 800, color: "#000000", fontSize: 12, }}>
                          {i + 1}
                        </td>
                        <td style={{ padding: "8px 10px", borderBottom: "1px solid #fef3c7", borderRight: "1px solid #fef3c7", fontWeight: 700, color: "#000000", fontSize: 12, whiteSpace: "nowrap", }}>
                          {row.name}
                        </td>
                        <td style={{ padding: "8px 10px", borderBottom: "1px solid #fef3c7", borderRight: "1px solid #fef3c7", fontWeight: 600, color: "#000000", fontSize: 11, whiteSpace: "nowrap", }}>
                          {row.designation}
                        </td>
                        <td style={{ padding: "4px 8px", borderBottom: "1px solid #fef3c7", borderRight: "1px solid #fef3c7", textAlign: "center", }}>
                          {isViewMode ? (
                            ...
                            <SelectWrapper disabled={false} color={val === "P" ? "#92400e" : val === "A" ? "#991b1b" : "#9ca3af"}>
                              <select
                                value={val}
                                onChange={(e) => updateAttendance(i, day, e.target.value)}
                        ...
                        {!isViewMode && (
                           <td style={{ padding: "4px", borderBottom: "1px solid #fef3c7", textAlign: "center", }}>
                             <button
                               onClick={() => removeRow(i)}
                               style={{ width: 24, height: 24, borderRadius: 6, border: "1px solid #fecaca", background: "#fff", color: "#dc2626", cursor: "pointer", fontWeight: 700, fontSize: 11, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto" }}
            ...
            >
              ...
              <div style={{width: '100%', '@media (min-width: 640px)': {width: 'auto'}}}>
                {isViewMode && (
                   <div>
                     <p style={{fontSize: 11, fontWeight: 700, color: '#64748b', textTransform: 'uppercase', marginBottom: 2}}>Prepared By</p>
                     <p style={{fontSize: 16, fontWeight: 900, color: '#d97706', textTransform: 'capitalize', margin: 0}}>{preparedBy}</p>
                   </div>
                )}
              </div>
                ...
                {saveMsg && (
                  <span
                    ...
                    style={{
                      ...
                      border: "1px solid #fcd34d", borderRadius: 8, padding: "5px 12px", whiteSpace: "nowrap",
                    }}
                  >
                    {saveMsg}
                  </span>

...
const Inspectionprint = ({ items = [], currentReport, onEditForm, onBack }) => {
 ...
 const urlPart = searchParams.get("part_name");
 const urlOp = searchParams.get("operation");
    ...
    if (urlCustomer && urlPart && urlDate) {
      ...
      fetch(
        `${API_URL}/get-inspection-report/?customer=${encodeURIComponent(
          urlCustomer,
        )}&part_name=${encodeURIComponent(
          urlPart,
        )}&operation=${encodeURIComponent(urlOp || "")}&date=${urlDate}`,
      )
        .then((res) => {
  ...
  const handleFilterCustomerChange = (e) => {
    ...
    if (cust !== "All Customers") {
      fetch(
        `${API_URL}/master-dropdown/?filter=part&cust=${encodeURIComponent(
          cust,
        )}`,
      )
        .then((res) => res.json())
  ...
  const handleFilterPartChange = (e) => {
    ...
    if (part !== "All Parts") {
      fetch(
        ...
        .then((data) =>
          setMasterLists((prev) => ({ ...prev, operations: data })),
        );
    } else {
  ...
  const handleApplyFilter = () => {
    navigate(
      `?customer=${encodeURIComponent(
        ...
        filterPartName,
      )}&operation=${encodeURIComponent(filterOperation)}&date=${filterDate}`,
    );
  ...
  const scheduleEntries = rawScheduleEntries.filter((entry) => {
    if (filterDate && filterDate !== urlDate)
      return (
        entry.date === filterDate ||
        formatDisplay(entry.date) === formatDisplay(filterDate)
      );
  ...
  const productItems = activeItems
    .filter((x) => x.sr_no >= 1 && x.sr_no <= 10)
    .sort((a, b) => a.sr_no - b.sr_no);

  const processItems = activeItems
    .filter((x) => x.sr_no >= 11 && x.sr_no <= 20)
    .sort((a, b) => a.sr_no - b.sr_no);

  const validProductItems = productItems.filter(
    (x) => x.item && x.item.trim() !== "",
  );

  const validProcessItems = processItems.filter(
    (x) => x.item && x.item.trim() !== "",
  );
  ...
  const buildScheduleRows = () => {
    ...
    scheduleEntries.forEach((entry) => {
      const groupKey = `${entry.operator || ""}__${entry.machine_no || ""}__${
        entry.date || ""
      }`;
    ...
    return Object.values(grouped).map((grp) => {
      ...
      sorted.forEach((e) => {
        ...
        if (!slotMap[si])
          slotMap[si] = { time_type: e.time_type || "SETUP", readings: [] };

        const vals =
          orderedValidItems.length > 0
      ...
      const slots = Object.entries(slotMap)
        ...
        .sort((a, b) => {
          const ta = timeOrder[a[1].time_type] ?? 9,
            tb = timeOrder[b[1].time_type] ?? 9;
          return ta !== tb ? ta - tb : Number(a[0]) - Number(b[0]);
        })
  ...
  const totalSchedHtmlRows = scheduleRows.reduce(
  ...
  const TH =
    "border border-black text-center align-middle font-bold bg-[#f5f5f5]";
    ...
    return (
      <div className="min-h-screen bg-[#f5f5f5] flex items-center justify-center">
        <div className="text-center">
          <div
            className="spinner-border text-[#3b5998] mb-3"
            role="status"
              ...
              <div className="p-4 flex flex-col gap-3">
                ...
                <div>
                  ...
                  >
                    ...
                    {masterLists.customers.map((c, i) => (
                      <option key={`cust-${i}`} value={c}>
                        {c}
                      </option>
                    ))}
                  </select>
                ...
                <div>
                  ...
                  <select
                    ...
                    disabled={filterCustomer === "All Customers"}
                    className="w-full border border-gray-300 rounded p-2 text-sm focus:outline-none focus:border-[#3b5998] text-gray-700"
                  >
                    <option value="All Parts">All Parts</option>

                    {masterLists.parts.map((p, i) => (
                      <option key={`part-${i}`} value={p}>
                        {p}
                      </option>
                    ))}
                  </select>
                ...
                <div>
                  ...
                  <select
                    ...
                    disabled={filterPartName === "All Parts"}
                    className="w-full border border-gray-300 rounded p-2 text-sm focus:outline-none focus:border-[#3b5998] text-gray-700"
                  >
                    <option value="All Operations">All Operations</option>

                    {masterLists.operations.map((o, i) => (
                      <option key={`op-${i}`} value={o}>
                        {o}
                      </option>
                    ))}
                  </select>
            ...
            <tr>
              <td className="p-0">
                    ...
                    <tr>
                      <td className="w-[120px] p-1.5 text-center border-r border-black align-middle bg-white">
                        <img
                          src={atomone}
                          alt="ATOM ONE"
                          className="max-w-full max-h-[55px] object-contain block mx-auto print:max-h-[42px]"
                        />
                      </td>
                      ...
                      <td className="w-[220px] p-0 border-l border-black align-middle bg-white">
                        <table className="w-full border-none border-collapse">
                          <tbody>
                            <tr>
                              <td className="border-b border-black font-bold w-[95px] border-r border-r-black px-2 py-1 text-[11px] h-[24px] text-left print-text-sm print:h-[21px]">
                                DOC NO:
                              </td>

                              <td className="border-b border-black font-semibold text-[13px] px-2 py-1 h-[24px] text-left print-text-lg print:h-[21px]">
                                {activeReport?.doc_no || "KGTL-QCL-01"}
                              </td>
                            </tr>
                            ...
                            <tr>
                              ...
                              <td className="border-b border-black font-semibold text-[13px] px-2 py-1 h-[24px] text-left print-text-lg print:h-[21px]">
                                {activeReport?.revision_no || "01"}
                              </td>
                            </tr>

                            <tr>
                              <td className="font-bold w-[95px] border-r border-r-black px-2 py-1 text-[11px] h-[24px] text-left print-text-sm print:h-[21px]">
                                DATE:
                              </td>

                              <td className="font-semibold text-[13px] px-2 py-1 h-[24px] text-left print-text-lg print:h-[21px]">
                                {displayDate}
                              </td>
                            </tr>
                          </tbody>
                ...
                <table className="w-full border-collapse border border-black">
                  <tbody>
                    <tr>
                      <td className="bg-[#f5f5f5] text-[11px] font-bold uppercase w-[13%] px-2.5 h-[32px] border border-black text-center align-middle print-text-sm print:h-[26px]">
                        CUSTOMER NAME
                      </td>

                      <td className="w-[37%] text-[12px] font-semibold text-left pl-2.5 px-2.5 h-[32px] border border-black align-middle print-text-lg print:h-[26px] bg-white">
                        {displayCustomer}
                      </td>
          ...
          <tbody>
            <tr>
              <td className="p-0 align-top">
                <table
                  className="w-full border-collapse border border-t-0 border-b-0 border-black table-fixed"
                  style={{ height: `${inspTotalH}px` }}
                >
                  <thead>
                    <tr style={{ height: `${INSP_THEAD}px` }}>
                      <th
                        className={`${TH} text-[12px] px-1 py-1.5 print:text-[9px] print:py-[1px]`}
                      >
                        SR. NO.
                      </th>
                  ...
                  <tbody>
                    {Array.from({ length: totalRows }, (_, i) => {
                      ...
                      const rSR = process?.item?.trim()
                        ? productCount + i + 1
                        : "";

                      const bgClass =
                        (i + 1) % 2 === 0 ? "bg-[#fafafa]" : "bg-white";
                      ...
                      return (
                        ...
                        <tr key={i} style={{ height: `${inspRowH}px` }}>
                          <td
                            className={`${TD} ${bgClass} text-[11px] p-1 print:text-[9px] print:p-0`}
                          >
                            {pSR}
                          </td>

                          <td
                            className={`${TD} ${bgClass} text-[11px] p-1 text-center font-semibold pl-[5px] print:text-[9px] print:p-0`}
                          >
                            {product?.item || ""}
                          </td>
                          ...
                          <td
                            className={`${TD} ${bgClass} text-[11px] p-1 print:text-[9px] print:p-0`}
                          >
                            {product?.tol || product?.tolerance || ""}
                          </td>

                          <td
                            className={`${TD} ${bgClass} text-[11px] p-1 print:text-[9px] print:p-0`}
                          >
                            {product?.instr || product?.inst || ""}
                          </td>
                          ...
                          <td
                            className={`${TD} ${bgClass} text-[11px] p-1 print:text-[9px] print:p-0`}
                          >
                            {process?.tol || process?.tolerance || ""}
                          </td>

                          <td
                            className={`${TD} ${bgClass} text-[11px] p-1 print:text-[9px] print:p-0`}
                          >
                            {process?.instr || process?.inst || ""}
                          </td>
                        </tr>
                      );
                    })}
                ...
                <table className="w-full border-collapse border border-black table-fixed">
                  ...
                  <thead>
                    <tr style={{ height: `${SCHED_THEAD}px` }}>
                      <th
                        className={`${TH} text-[11px] p-1 whitespace-normal break-words leading-[1.2] print:text-[8px] print:leading-[1.1] print:p-[1px]`}
                      >
                        SR
                      </th>
                      ...
                      {Array.from({ length: totalFilledCols }, (_, i) => (
                        <th
                          key={i}
                          className={`${TH} text-[11px] p-1 print:text-[8px] print:p-[1px]`}
                        >
                          {i + 1}
                        </th>
                      ))}
                      ...
                      const timeSpans = si.slots.map((row, idx) => {
                        ...
                        if (!prev || prev.slotKey !== row.slotKey) {
                          ...
                          for (
                            let j = idx;
                            j < si.slots.length &&
                            si.slots[j].slotKey === row.slotKey;
                            j++
                          )
                            count++;
                        ...
                        <React.Fragment key={rowIdx}>
                          {si.slots.map((row, timeIdx) => {
                            ...
                            return (
                              <tr
                                key={`${rowIdx}-${timeIdx}`}
                                style={{ height: `${SCHED_ROW_H}px` }}
                                className="bg-white break-inside-avoid"
                              >
                                {timeIdx === 0 && (
                                  <td
                                    rowSpan={totalTimeRows}
                                    className={`${TD} font-bold text-[10px] print:text-[8.5px] p-0.5`}
                                  >
                                    {si.sr}
                                  </td>
                                )}
                                ...
                                {timeIdx === 0 && (
                                  <td
                                    rowSpan={totalTimeRows}
                                    className={`${TD} text-[8px] whitespace-normal leading-[1.2] print:text-[8.5px] p-0.5`}
                                  >
                                    {si.date || ""}
                                  </td>
                                )}
                                ...
                                {timeIdx === 0 && (
                                  <td
                                    rowSpan={totalTimeRows}
                                    className={`${TD} text-[9px] print:text-[8.5px] p-0.5`}
                                  >
                                    {si.mcNo || ""}
                                  </td>
                                )}
                                ...
                                {span > 0 && (
                                  <td
                                    rowSpan={span}
                                    className={`${TD} font-bold text-[8.5px] print:text-[8.5px] p-0.5`}
                                  >
                                    {row.time}
                                  </td>
                                )}
                    ...
                    <tr>
                      <td className="w-1/2 border-r border-black px-3 py-3 align-bottom">
                        <div className="flex items-end">
                          <span className="font-bold text-[12px] whitespace-nowrap mr-2 text-black print:text-[10px]">
                            PREPARED BY:
                          </span>

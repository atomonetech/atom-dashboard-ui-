...
const CoherenceChecklist = () => {
    ...
    // Initial Fetch (Parts List & Default Date)
     // Fetch Existing Report (View Mode)
    useEffect(() => {
        if (!id) return;

        const fetchReport = async () => {
            try {
                    const res = await axios.get(`${API_BASE_URL}/api/get-single-report/coherence-view/${id}/`);
                
                if (res.data.success) {
                    const data = res.data.data || {};

                    setFormData({
                        // Changed to match the camelCase keys from your API response
                        partName: data.partName || '',
                        partNo: data.partNo || '',
                        date: data.date || '',
                        model: data.model || '',
                        preparedBy: data.preparedBy || data.Prepared_By || data["Prepared By"] || '', // Handling exact API output
                        verifiedBy: data.verifiedBy || '',
                        
                        // Map operations from backend back to frontend state
                        operations: data.operations && data.operations.length > 0 
                            ? data.operations.map(op => ({
                                processName: op.processName || '',
                                machineJig: op.machineJig || '',
                                pfd: op.pfd || '',
                                pfmea: op.pfmea || '',
                                specOSWI: op.specOSWI || '',
                                specStd: op.specStd || '',
                                inspectionFIS: op.inspectionFIS || '',
                                remarks: op.remarks || ''
                            }))
                            : [emptyOperation]
                    });

                    setApprovalRemark(data.Approver_remarks && data.Approver_remarks !== "Null" ? data.Approver_remarks : "");
                    setApprovalStatus(data.approval_status || "");
                    setReviewedAt(data.approved_or_rejected_at || "");
                    
                } else {
                    errorAlert(res.data.error || "Failed to load the Control Plan.");
                }
            } catch (err) {
                console.error("Error loading Report:", err);
                errorAlert(err.response?.data?.error || "Failed to load the Control Plan.");
            }
        };
